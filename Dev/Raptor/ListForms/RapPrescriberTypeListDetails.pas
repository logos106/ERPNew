unit RapPrescriberTypeListDetails;
...
uses
  ...
  StdCtrls, DNMPanel, ComCtrls, DNMSpeedButton, kbmMemTable,
  CustomInputBox, wwdblook, Shader;

type
  TRapPrescriberTypeListDetailsGUI = class(TBaseListingGUI)
...
uses FastFuncs,CommonLib, CommonDbLib, QueryLoadObj, BaseInputForm,
  RapPrescriberTypeList;

{$R *.dfm}

Procedure TRapPrescriberTypeListDetailsGUI.REfreshQuery;
var
    QLO :TQueryLoad;
    Dates :TVarParams;
...
procedure TRapPrescriberTypeListDetailsGUI.grpFiltersClick(Sender: TObject);
begin
...
procedure TRapPrescriberTypeListDetailsGUI.grdMainCalcCellColors(Sender: TObject;
...
procedure TRapPrescriberTypeListDetailsGUI.cmdNewClick(Sender: TObject);
Var Form : TComponent;
begin
  inherited;
...
procedure TRapPrescriberTypeListDetailsGUI.actExecuteFilterExecute(Sender: TObject);
begin
  inherited;
...
procedure TRapPrescriberTypeListDetailsGUI.FormCreate(Sender: TObject);
begin
  NoDateRange := true;
  inherited;
  LabelGridFromFieldNames := False;
...
procedure TRapPrescriberTypeListDetailsGUI.FormClose(Sender: TObject;
...
procedure TRapPrescriberTypeListDetailsGUI.grpProductdetailsClick(
  Sender: TObject);
var
    form :TRapPrescriberTypeListGUI;
begin
  inherited;
    if grpProductdetails.Itemindex = 1 then Exit;
    Form := TRapPrescriberTypeListGUI(GetComponentByClassName('TRapPrescriberTypeListGUI'));
    if Assigned(form) then begin
        Form.FormStyle := fsMDIChild;
        form.BringTofront;
        form.grpProductdetails.ItemIndex := 0;
...
initialization
  RegisterClassOnce(TRapPrescriberTypeListDetailsGUI);
