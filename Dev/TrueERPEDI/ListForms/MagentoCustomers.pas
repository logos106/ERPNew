...
type
  TMagentoCustomersGUI = class(TMagentoListBaseGUI)
...
procedure TMagentoCustomersGUI.actRefreshQryExecute(Sender: TObject);
begin
  inherited;
  if not FConnected then exit;
end;

procedure TMagentoCustomersGUI.btnSynchClick(Sender: TObject);
var
  sJSON : string;
  AFields: TJSONArray;
  AJSON: TJSONObject;
...
procedure TMagentoCustomersGUI.FormCreate(Sender: TObject);
begin
  inherited;
  FErrorMessage       := '';
...
procedure TMagentoCustomersGUI.FormShow(Sender: TObject);
begin
  inherited;
  if not FConnected then exit;
...
procedure TMagentoCustomersGUI.RefreshQuery;
begin
  inherited;
...
procedure TMagentoCustomersGUI.SetGridColumns;
begin
  inherited;
...
procedure TMagentoCustomersGUI.SynchTransactions;
begin
//  inherited;
...
procedure TMagentoCustomersGUI.wsJobClientException(
...
procedure TMagentoCustomersGUI.wsJobClientMessage(Connection: TsgcWSConnection;
  const Text: string);
var
...
Function TMagentoCustomersGUI.Autocheck :boolean;
begin
  result := False;
end;

Function TMagentoCustomersGUI.CheckOrdersInterval:Integer;
begin
  Result := MAGENTO_CHECKCUSTOMERSINTERVAL;
end;

initialization
  RegisterClass(TMagentoCustomersGUI);
