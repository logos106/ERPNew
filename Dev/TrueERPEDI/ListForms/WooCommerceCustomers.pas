...
type
  TWooCommerceCustomersGUI = class(TMagentoListBaseGUI)
...
uses AppEnvironment, CommonLib, BusobjExternalXRef, ErpSynchMagento, utCoreEDIConst, coreEDIUtils,
tcConst;

{$R *.dfm}
{ TMagentoCustomersGUI }

procedure TWooCommerceCustomersGUI.actRefreshQryExecute(Sender: TObject);
begin
  inherited;
  if not FConnected then exit;
end;

procedure TWooCommerceCustomersGUI.btnSynchClick(Sender: TObject);
var
  sJSON : string;
  AFields: TJSONArray;
  AJSON: TJSONObject;
begin
  ...
  try
    AJSON := JO;
    AJSON.S['JOBSEGMENT'] := 'WOOC_SYNCH_CUSTOMERS';
    AFields.Add(AJSON);
...
procedure TWooCommerceCustomersGUI.FormCreate(Sender: TObject);
begin
  inherited;
  FErrorMessage       := '';
...
procedure TWooCommerceCustomersGUI.FormShow(Sender: TObject);
begin
  inherited;
  if not FConnected then exit;
...
procedure TWooCommerceCustomersGUI.RefreshQuery;
begin
  inherited;
  Qrymain.parambyname('Externaltype').asString:= Servicename_WooComm; //SynchObj.CoreService.ServiceName;
  Qrymain.parambyname('ObjectType').asString:= ExRef_Objtype_Customer;
end;

procedure TWooCommerceCustomersGUI.SetGridColumns;
begin
  inherited;
...
procedure TWooCommerceCustomersGUI.SynchTransactions;
begin
//  inherited;
...
procedure TWooCommerceCustomersGUI.wsJobClientException(
...
procedure TWooCommerceCustomersGUI.wsJobClientMessage(Connection: TsgcWSConnection;
  const Text: string);
var
...
Function TWooCommerceCustomersGUI.Autocheck :boolean;
begin
  result := False;
end;

Function TWooCommerceCustomersGUI.CheckOrdersInterval:Integer;
begin
  Result := MAGENTO_CHECKCUSTOMERSINTERVAL;
end;

initialization
  RegisterClass(TWooCommerceCustomersGUI);
