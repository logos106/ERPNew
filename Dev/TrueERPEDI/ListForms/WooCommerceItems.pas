...
type
  TWooCommerceItemsGUI = class(TMagentoListBaseGUI)
...
{$R *.dfm}
procedure TWooCommerceItemsGUI.SetGridColumns;
begin
  inherited;
  RemoveFieldfromGrid(QrymainPartsId.fieldname);
end;

procedure TWooCommerceItemsGUI.SynchTransactions;
begin
//  inherited;
...
procedure TWooCommerceItemsGUI.btnSynchClick(Sender: TObject);
var
  sJSON : string;
  AFields: TJSONArray;
  AJSON: TJSONObject;
begin
  ...
  try
    AJSON := JO;
    AJSON.S['JOBSEGMENT'] := 'WOOC_SYNCH_PRODUCTS';
    AFields.Add(AJSON);
    sJSON := BuildJSON('JOB', 0, 0, 0, AFields, 'TrueERP', 'WooCommerce');
  finally
    AFields.Free;
  end;
...
procedure TWooCommerceItemsGUI.btnUpLoadClick(Sender: TObject);
var
  i : Integer;
  oStr: TStringList;
  sJSON : string;
...
begin
  ...
  try
    ...
    AJSON := JO;
    AJSON.S['JOBSEGMENT'] := 'WOOC_SYNCH_PRODUCTS_BULK';
    AFields.Add(AJSON);
    AJSONIds := JO;
...
procedure TWooCommerceItemsGUI.FormCreate(Sender: TObject);
begin
  inherited;
  ProductsForMagentoSQL('', ServiceName_WooComm, true, Qrymain.sql);
...
procedure TWooCommerceItemsGUI.FormShow(Sender: TObject);
begin
  inherited;
  ProductsForMagentoSQL('', Servicename_WooComm, true, Qrymain.sql);
  RefreshQuery;
  ...
  EnableMultiSelect('Select Products to Upload to WooCommerce. (Changes to an Uploaded Product will be Ignored)');
  Qrymain.First;
...
procedure TWooCommerceItemsGUI.grpFiltersClick(Sender: TObject);
begin
...
procedure TWooCommerceItemsGUI.IterateselectedRecordsCallback(var Abort: boolean; SelIndex: Integer);
begin
...
procedure TWooCommerceItemsGUI.WriteGuiPrefExtra;
begin
  inherited;
  GuiPrefs.Node['Options.PartsID'].asInteger         := QrymainpartsID.asInteger;
end;
procedure TWooCommerceItemsGUI.wsJobClientException(
...
procedure TWooCommerceItemsGUI.wsJobClientMessage(
...
procedure TWooCommerceItemsGUI.RefreshQuery;
begin
//  inherited;
  qryMain.close;
  qrymain.Open;
...
procedure TWooCommerceItemsGUI.actRefreshQryExecute(Sender: TObject);
begin
  inherited;
  if not FConnected then exit;
end;

Function TWooCommerceItemsGUI.Autocheck :boolean;
begin
  result := False;
end;

Function TWooCommerceItemsGUI.CheckOrdersInterval:Integer;
begin
  Result := MAGENTO_CHECKPRODUCTSINTERVAL;
end;

initialization
  RegisterClass(TWooCommerceItemsGUI);
