...
type
  TWooCommerceInvoicesGUI = class(TMagentoListBaseGUI)
...
procedure TWooCommerceInvoicesGUI.FormCreate(Sender: TObject);
begin
...
procedure TWooCommerceInvoicesGUI.FormShow(Sender: TObject);
begin
  inherited;
...
procedure TWooCommerceInvoicesGUI.grdMainDblClick(Sender: TObject);
begin
  SubsequentID := Chr(95) + qryMainTransType.AsString;
  inherited;
end;

procedure TWooCommerceInvoicesGUI.RefreshQuery;
begin
  InitDateFromnDateto;
  inherited;
end;

Function TWooCommerceInvoicesGUI.Autocheck :boolean;
begin
  result := False; //Appenv.companyprefs.MAgentoConfig.AutosynchList;
end;

Function TWooCommerceInvoicesGUI.CheckOrdersInterval:Integer;
begin
  Result := MAGENTO_CHECKORDERSINTERVAL;
end;
procedure TWooCommerceInvoicesGUI.AssignKeyId(Popupform: TBaseInputGUI);
begin
  inherited;
end;
procedure TWooCommerceInvoicesGUI.DoBusinessObjectEvent(const Sender: TDatasetBusObj;const EventType, Value: string);
begin

end;
procedure TWooCommerceInvoicesGUI.btnSynchClick(Sender: TObject);
var
  sJSON : string;
  AFields: TJSONArray;
  AJSON: TJSONObject;
begin
  ...
  try
    AJSON := JO;
    AJSON.S['JOBSEGMENT'] := 'WOOC_SYNCH_ORDERS';
    AFields.Add(AJSON);
...
procedure TWooCommerceInvoicesGUI.SynchTransactions;
var
...
procedure TWooCommerceInvoicesGUI.wsJobClientException(Connection: TsgcWSConnection;
  E: Exception);
begin
  inherited;
...
procedure TWooCommerceInvoicesGUI.wsJobClientMessage(Connection: TsgcWSConnection;
  const Text: string);
var
...
initialization
  RegisterClass(TWooCommerceInvoicesGUI);
