...
type
  TProductionOrderListGUI = class(TBaseListingGUI)
...
{ TProductionOrderListGUI }

procedure TProductionOrderListGUI.AfterPopup(Popupform: TComponent);
begin
  inherited;
...
procedure TProductionOrderListGUI.chkHideRawMaterialsClick(Sender: TObject);
begin
  inherited;
...
procedure TProductionOrderListGUI.cmdCloseClick(Sender: TObject);
begin
...
procedure TProductionOrderListGUI.cmdPrintClick(Sender: TObject);
begin
  (*inherited;
  initReporttoPrint;*)
  fsReportSQL:= '';
...
procedure TProductionOrderListGUI.edtcompleteQtyDblClick(Sender: TObject);
begin
  inherited;
...
procedure TProductionOrderListGUI.FormCreate(Sender: TObject);
var
...
begin
  ...
  Qrymain.SQL.add('Where ((:HideRawMaterial ="T" and ifnull(PTID'+LvlNo(fiBomLevels)+',0) =0) or (:HideRawMaterial ="F" ))');
  ...
  for level := 1 to fiBomLevels do begin
      ...
      with TFloatfield.Create(Self) do begin
        fieldKind := fkData;
        fieldname := 'totalQty'+LvlNo(Level);
        name := ComponentName(self, 'Qrymain'+fieldname);
        DataSet := qryMain;
        visible := False;
      end;
      ...
      with TIntegerfield.Create(Self) do begin
        fieldKind := fkData;
        fieldname := 'SequenceUp'+LvlNo(Level);
        name := ComponentName(self, 'Qrymain'+fieldname);
        DataSet := qryMain;
        visible := False;
      end;
      with TIntegerfield.Create(Self) do begin
        fieldKind := fkData;
        fieldname := 'ParentID'+LvlNo(Level);
        name := ComponentName(self, 'Qrymain'+fieldname);
        DataSet := qryMain;
        visible := False;
      end;
...
procedure TProductionOrderListGUI.FormDestroy(Sender: TObject);
begin
...
procedure TProductionOrderListGUI.FormShow(Sender: TObject);
begin
  inherited;
    lblNote.caption := '"Double Click on ' + quotedstr(replacestr(qrymaincompleteqty.DisplayLabel , '~' , ''))+' Field to Complete the Remaining Qty."';
    lblNote.REfresh;
    lblNote.visible := True;

end;

function TProductionOrderListGUI.GetReportTypeID: integer;
begin
  REsult := GetTemplateTypeId('Sales Order');
end;

procedure TProductionOrderListGUI.grdMainCalcCellColors(Sender: TObject;
...
begin
  ...
  try
    ...
    if qryMaintobeBuitQty.asFloat >0 then
      if sametext(Field.Fieldname, qryMainCompleteQty.fieldname)  then
...
procedure TProductionOrderListGUI.grdMainColExit(Sender: TObject);
begin
  inherited;
  if sametext(activefieldname, qrymaincompleteqty.fieldname) and (qrymaincompleteqty.asFloat <> 0) then
    OncompleteQtyChange;
end;

procedure TProductionOrderListGUI.grdMainDblClick(Sender: TObject);
begin
  if sametext(activefieldname ,qryMaincompleteQty.fieldname) then begin
...
procedure TProductionOrderListGUI.grdMainExit(Sender: TObject);
begin
  inherited;
  if sametext(activefieldname, qrymaincompleteqty.fieldname) and (qrymaincompleteqty.asFloat <> 0) then
    OncompleteQtyChange;
...
procedure TProductionOrderListGUI.grdMainKeyPress(Sender: TObject;
  var Key: Char);
begin
  // binny : completeQty is editable in this form
  try
  if sametext(activefieldname, qrymaincompleteqty.fieldname) then else
  inherited;
  except
...
procedure TProductionOrderListGUI.grpFiltersClick(Sender: TObject);
begin
...
procedure TProductionOrderListGUI.RefreshQuery;
begin
  (*InitDateFromnDateto;
...
procedure TProductionOrderListGUI.qryMainAfterPost(DataSet: TDataSet);
begin
  inherited;
  OncompleteQtyChange;
end;
Procedure TProductionOrderListGUI.PopulatereportSQL(const CompletingQty:Double);
begin
...
procedure TProductionOrderListGUI.OncompleteQtyChange;
var
  fd:Double;
...
begin
  ...
  try
      ...
      Try
          ...
          fbUpdatingQty := True;
          try
          if qrymaincompleteQty.asfloat >0 then begin
            if not Userlock.Lock('tblsales' , qrymainsaleId.asInteger , 'ProductionOrderList') then begin
              ...
              editDb(qrymain);
              qrymaincompleteQty.asfloat := 0;
              PostDb(qrymain);
              Exit;
            end;
            try
              fd:= qrymaincompleteQty.asfloat;
              ...
              try
                ...
                grdmain.SetActiveField(qrymaincompleteQty.fieldname);
              end;
            finally
...
procedure TProductionOrderListGUI.initCorresforPrintTemplateOptionsReport(Sender: TTemplateOptions; var OutGoingFileName: String;var Corres: TCorrespondenceGui);
begin
  inherited;
...
procedure TProductionOrderListGUI.initPrintTemplateOptionsReport(Sender: TTemplateOptions;
                                                   var  ssql:String;
...
procedure TProductionOrderListGUI.initReporttoPrint;
var
  s:String;
begin
  s:= '';
...
procedure TProductionOrderListGUI.qryMainBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  Qrymain.parambyname('HideRawMaterial').asString := Booleantostr(chkHideRawMaterials.checked);
end;

procedure TProductionOrderListGUI.qryMainBeforePost(DataSet: TDataSet);
begin
  inherited;
  if qrymaincompleteQty.asfloat <0 then qrymaincompleteQty.asfloat := 0;
end;

procedure TProductionOrderListGUI.SetGridColumns;
begin
  inherited;
...
procedure TProductionOrderListGUI.MakeTable;
var
  fiBomLevels, level:Integer;
...
procedure TProductionOrderListGUI.ReadnApplyGuiPrefExtra;
begin
  inherited;
...
Procedure TProductionOrderListGUI.WriteGuiPrefExtra;
begin
  inherited;
...
initialization
  RegisterClassOnce(TProductionOrderListGUI);
