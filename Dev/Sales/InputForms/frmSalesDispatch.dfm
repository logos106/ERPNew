inherited fmSalesDispatch: TfmSalesDispatch
  Left = 821
  Top = 170
  HelpContext = 1290002
  Caption = 'Product Dispatch'
  ClientHeight = 502
  ClientWidth = 618
  ExplicitLeft = 821
  ExplicitTop = 170
  ExplicitWidth = 634
  ExplicitHeight = 541
  PixelsPerInch = 96
  TextHeight = 15
  inherited lblSkingroupMsg: TLabel
    Top = 410
    Width = 618
    HelpContext = 1290003
    ExplicitTop = 223
    ExplicitWidth = 455
  end
  inherited shapehint: TShape
    Left = 73
    Top = 10
    ExplicitLeft = 49
    ExplicitTop = 10
  end
  inherited imgGridWatermark: TImage
    Left = 305
    Top = 10
    ExplicitLeft = 305
    ExplicitTop = 10
  end
  inherited shapehintextra1: TShape
    Left = 159
    Top = 10
    ExplicitLeft = 113
    ExplicitTop = 10
  end
  object DNMPanel1: TDNMPanel [5]
    Left = 0
    Top = 205
    Width = 618
    Height = 205
    HelpContext = 1290004
    Align = alClient
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 3
    object grdTransactions: TwwDBGrid
      Left = 1
      Top = 1
      Width = 616
      Height = 203
      HelpContext = 1290005
      Selected.Strings = (
        'SaleID'#9'5'#9'Sale #'
        'ProductName'#9'18'#9'Product'
        'BatchNo'#9'18'#9'Batch No'#9'F'
        'UOMQty'#9'5'#9'Qty')
      MemoAttributes = [mSizeable, mWordWrap, mGridShow]
      IniAttributes.Delimiter = ';;'
      TitleColor = clWhite
      FixedCols = 0
      ShowHorzScrollBar = True
      EditControlOptions = [ecoCheckboxSingleClick, ecoSearchOwnerForm]
      Align = alClient
      DataSource = dsSalesDispatchLines
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -21
      Font.Name = 'Arial'
      Font.Style = []
      KeyOptions = []
      Options = [dgEditing, dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgWordWrap, dgRowResize]
      ParentFont = False
      TabOrder = 0
      TitleAlignment = taLeftJustify
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -21
      TitleFont.Name = 'Arial'
      TitleFont.Style = [fsBold]
      TitleLines = 1
      TitleButtons = True
      OnExit = grdTransactionsExit
      FooterColor = clWhite
      FooterCellColor = clWhite
      PaintOptions.BackgroundOptions = [coBlendFixedRow]
      object btnDelete: TwwIButton
        Left = -1
        Top = 0
        Width = 22
        Height = 28
        HelpContext = 1290006
        AllowAllUp = True
        Glyph.Data = {
          
          E0E3FFFFFFFFFFFF0000}
        Layout = blGlyphBottom
        OnClick = btnDeleteClick
      end
    end
  end
  object DNMPanel2: TDNMPanel [6]
    Left = 0
    Top = 0
    Width = 618
    Height = 59
    HelpContext = 1290007
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 0
    DesignSize = (
      618
      59)
    object pnlTitle: TDNMPanel
      Left = 114
      Top = 7
      Width = 391
      Height = 47
      HelpContext = 1290008
      Anchors = [akLeft, akTop, akRight]
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -29
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Locked = True
      ParentFont = False
      TabOrder = 0
      object TitleShader: TShader
        Left = 1
        Top = 1
        Width = 389
        Height = 45
        HelpContext = 1290009
        Align = alClient
        BevelOuter = bvNone
        TabOrder = 0
        FromColor = clBtnFace
        ToColor = clWhite
        FromColorMirror = clWhite
        ToColorMirror = clBtnFace
        Steps = 10
        Direction = False
        Version = '1.4.0.0'
        object TitleLabel: TLabel
          Left = 0
          Top = 0
          Width = 389
          Height = 45
          HelpContext = 1290010
          Align = alClient
          Alignment = taCenter
          AutoSize = False
          Caption = 'Product Dispatch'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -37
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          ParentFont = False
          Transparent = True
          Layout = tlCenter
          ExplicitWidth = 171
          ExplicitHeight = 37
        end
      end
    end
  end
  object DNMPanel3: TDNMPanel [7]
    Left = 0
    Top = 410
    Width = 618
    Height = 92
    HelpContext = 1290011
    Align = alBottom
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 4
    DesignSize = (
      618
      92)
    object cmdClose: TDNMSpeedButton
      Left = 62
      Top = 52
      Width = 106
      Height = 29
      HelpContext = 1290012
      Anchors = [akBottom]
      Caption = '&Save'
      DisableTransparent = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000}
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -12
      HotTrackFont.Name = 'Arial'
      HotTrackFont.Style = []
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = cmdCloseClick
    end
    object cmdCancel: TDNMSpeedButton
      Left = 450
      Top = 52
      Width = 106
      Height = 29
      HelpContext = 1290013
      Anchors = [akBottom]
      Caption = '&Cancel'
      DisableTransparent = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      Glyph.Data = {
        
        0000}
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -12
      HotTrackFont.Name = 'Arial'
      HotTrackFont.Style = []
      ModalResult = 2
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
      OnClick = cmdCancelClick
    end
    object btnNext: TDNMSpeedButton
      Left = 191
      Top = 52
      Width = 106
      Height = 29
      HelpContext = 1290014
      Anchors = [akTop, akRight]
      Caption = '&New'
      DisableTransparent = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -11
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      TabStop = False
      OnClick = btnNextClick
    end
    object btnDel: TDNMSpeedButton
      Left = 320
      Top = 52
      Width = 106
      Height = 29
      HelpContext = 1290015
      Anchors = [akTop, akRight]
      Caption = 'Delivery Docket'
      DisableTransparent = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      HotTrackFont.Charset = DEFAULT_CHARSET
      HotTrackFont.Color = clWindowText
      HotTrackFont.Height = -11
      HotTrackFont.Name = 'MS Sans Serif'
      HotTrackFont.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      TabStop = False
      OnClick = btnDelClick
    end
  end
  object pnlBatch: TDNMPanel [8]
    Left = 0
    Top = 115
    Width = 618
    Height = 90
    HelpContext = 1290016
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 2
    object lblBatch: TLabel
      Left = 1
      Top = 1
      Width = 616
      Height = 34
      HelpContext = 1290017
      Align = alTop
      Alignment = taCenter
      Caption = 'Scan the Barcode'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -29
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
      ExplicitWidth = 244
    end
    object edtBatch: TEdit
      AlignWithMargins = True
      Left = 7
      Top = 38
      Width = 604
      Height = 48
      HelpContext = 1290018
      Margins.Left = 6
      Margins.Right = 6
      TabStop = False
      Align = alClient
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -29
      Font.Name = 'Arial'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      OnExit = edtBatchExit
      OnKeyPress = edtBatchKeyPress
      ExplicitHeight = 41
    end
  end
  object DNMPanel5: TDNMPanel [9]
    Left = 0
    Top = 59
    Width = 618
    Height = 56
    HelpContext = 1290019
    Align = alTop
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
    TabOrder = 1
    object SaleDate_Label: TLabel
      AlignWithMargins = True
      Left = 5
      Top = 13
      Width = 159
      Height = 29
      HelpContext = 1290020
      Margins.Left = 11
      Margins.Top = 0
      Margins.Right = 0
      Margins.Bottom = 0
      Alignment = taRightJustify
      Caption = 'Dispatch Date'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -24
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
      Layout = tlCenter
    end
    object Label48: TLabel
      Left = 368
      Top = 13
      Width = 113
      Height = 29
      HelpContext = 1290021
      Alignment = taRightJustify
      Caption = 'Employee'
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -24
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      Transparent = True
      WordWrap = True
    end
    object dtDispatchdate: TwwDBDateTimePicker
      Left = 172
      Top = 9
      Width = 177
      Height = 37
      HelpContext = 1290022
      AutoFillDateAndTime = True
      CalendarAttributes.Font.Charset = DEFAULT_CHARSET
      CalendarAttributes.Font.Color = clWindowText
      CalendarAttributes.Font.Height = -11
      CalendarAttributes.Font.Name = 'MS Sans Serif'
      CalendarAttributes.Font.Style = []
      DataField = 'DispatchDate'
      DataSource = dsSalesDispatch
      Epoch = 1950
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -24
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      ParentFont = False
      ShowButton = True
      TabOrder = 0
    end
    object cboEmployee: TwwDBLookupCombo
      Left = 487
      Top = 9
      Width = 126
      Height = 37
      HelpContext = 1290023
      Font.Charset = ANSI_CHARSET
      Font.Color = clWindowText
      Font.Height = -24
      Font.Name = 'Arial'
      Font.Style = [fsBold]
      DropDownAlignment = taLeftJustify
      Selected.Strings = (
        'EmployeeName'#9'25'#9'EmployeeName'#9'F')
      DataField = 'EmployeeName'
      DataSource = dsSalesDispatch
      LookupTable = cboEmployeeLookup
      LookupField = 'EmployeeName'
      DropDownWidth = 300
      ParentFont = False
      TabOrder = 1
      AutoDropDown = True
      ShowButton = True
      PreciseEditRegion = False
      AllowClearKey = True
      OnCloseUp = cboEmployeeCloseUp
    end
  end
  inherited tmrProcessMessage: TTimer
    Left = 125
    Top = 229
  end
  inherited tmrdelay: TTimer
    Left = 290
    Top = 229
  end
  inherited popSpelling: TPopupMenu
    Left = 92
    Top = 229
  end
  inherited dlgReportSelect: TSelectionDialog
    Left = 158
    Top = 229
  end
  inherited ApplicationEvents1: TApplicationEvents
    Left = 323
    Top = 229
  end
  inherited MyConnection: TERPConnection
    Server = 'localhost'
    Connected = True
    Left = 30
    Top = 199
  end
  inherited DataState: TDataState
    Left = 224
    Top = 229
  end
  inherited popCustomiseGrid: TAdvPopupMenu
    Left = 356
    Top = 229
  end
  inherited imgsort: TImageList
    Left = 257
    Top = 229
    Bitmap = {
      
      000000000000}
  end
  inherited QryCustomField: TERPQuery
    Left = 59
    Top = 229
  end
  inherited ProgressDialogOBJ: TProgressDialog
    Left = 389
    Top = 229
  end
  inherited mnuBusobjProperties: TAdvPopupMenu
    Left = 191
    Top = 229
  end
  inherited qryMemTrans: TERPQuery
    Left = 26
    Top = 229
  end
  object QrySalesDispatch: TERPQuery
    Connection = MyConnection
    SQL.Strings = (
      'SELECT * FROM tblSalesDispatch')
    Left = 27
    Top = 261
    object QrySalesDispatchID: TIntegerField
      AutoGenerateValue = arAutoInc
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'tblsalesdispatch.ID'
    end
    object QrySalesDispatchGlobalRef: TWideStringField
      DisplayWidth = 255
      FieldName = 'GlobalRef'
      Origin = 'tblsalesdispatch.GlobalRef'
      Size = 255
    end
    object QrySalesDispatchDispatchDate: TDateTimeField
      DisplayWidth = 18
      FieldName = 'DispatchDate'
      Origin = 'tblsalesdispatch.DispatchDate'
    end
    object QrySalesDispatchmstimestamp: TDateTimeField
      DisplayWidth = 18
      FieldName = 'mstimestamp'
      Origin = 'tblsalesdispatch.mstimestamp'
    end
    object QrySalesDispatchmsUpdateSitecode: TWideStringField
      DisplayWidth = 3
      FieldName = 'msUpdateSitecode'
      Origin = 'tblsalesdispatch.msUpdateSitecode'
      Size = 3
    end
    object QrySalesDispatchEmployeeID: TIntegerField
      FieldName = 'EmployeeID'
      Origin = 'tblsalesdispatch.EmployeeID'
    end
    object QrySalesDispatchEmployeeName: TWideStringField
      FieldName = 'EmployeeName'
      Origin = 'tblsalesdispatch.EmployeeName'
      Size = 100
    end
  end
  object dsSalesDispatch: TDataSource
    DataSet = QrySalesDispatch
    Left = 27
    Top = 297
  end
  object QrySalesDispatchLines: TERPQuery
    Connection = MyConnection
    SQL.Strings = (
      'SELECT * FROM tblSalesDispatchLines')
    Left = 67
    Top = 261
    object QrySalesDispatchLinesSaleID: TIntegerField
      DisplayLabel = 'Sale #'
      DisplayWidth = 5
      FieldName = 'SaleID'
      Origin = 'tblsalesdispatchlines.SaleID'
    end
    object QrySalesDispatchLinesProductName: TWideStringField
      DisplayLabel = 'Product'
      DisplayWidth = 18
      FieldName = 'ProductName'
      Origin = 'tblsalesdispatchlines.ProductName'
      Size = 100
    end
    object QrySalesDispatchLinesBatchNo: TWideStringField
      DisplayLabel = 'Batch No'
      DisplayWidth = 18
      FieldName = 'BatchNo'
      Origin = 'tblsalesdispatchlines.BatchNo'
      Size = 50
    end
    object QrySalesDispatchLinesUOMQty: TFloatField
      DisplayLabel = 'Qty'
      DisplayWidth = 5
      FieldName = 'UOMQty'
      Origin = 'tblsalesdispatchlines.UOMQty'
    end
    object QrySalesDispatchLinesID: TIntegerField
      AutoGenerateValue = arAutoInc
      DisplayWidth = 10
      FieldName = 'ID'
      Origin = 'tblsalesdispatchlines.ID'
      Visible = False
    end
    object QrySalesDispatchLinesGlobalRef: TWideStringField
      DisplayWidth = 255
      FieldName = 'GlobalRef'
      Origin = 'tblsalesdispatchlines.GlobalRef'
      Visible = False
      Size = 255
    end
    object QrySalesDispatchLinesDispatchID: TIntegerField
      DisplayWidth = 10
      FieldName = 'DispatchID'
      Origin = 'tblsalesdispatchlines.DispatchID'
      Visible = False
    end
    object QrySalesDispatchLinesSaleLineID: TIntegerField
      DisplayWidth = 10
      FieldName = 'SaleLineID'
      Origin = 'tblsalesdispatchlines.SaleLineID'
      Visible = False
    end
    object QrySalesDispatchLinesQty: TFloatField
      DisplayWidth = 10
      FieldName = 'Qty'
      Origin = 'tblsalesdispatchlines.Qty'
      Visible = False
    end
    object QrySalesDispatchLinesmstimestamp: TDateTimeField
      DisplayWidth = 18
      FieldName = 'mstimestamp'
      Origin = 'tblsalesdispatchlines.mstimestamp'
      Visible = False
    end
    object QrySalesDispatchLinesmsUpdateSitecode: TWideStringField
      DisplayWidth = 3
      FieldName = 'msUpdateSitecode'
      Origin = 'tblsalesdispatchlines.msUpdateSitecode'
      Visible = False
      Size = 3
    end
    object QrySalesDispatchLinesProductID: TIntegerField
      FieldName = 'ProductID'
      Origin = 'tblsalesdispatchlines.ProductID'
      Visible = False
    end
    object QrySalesDispatchLinesUOMID: TIntegerField
      FieldName = 'UOMID'
      Origin = 'tblsalesdispatchlines.UOMID'
      Visible = False
    end
    object QrySalesDispatchLinesUOMMultiplier: TFloatField
      DisplayWidth = 10
      FieldName = 'UOMMultiplier'
      Origin = 'tblsalesdispatchlines.UOMMultiplier'
      Visible = False
    end
    object QrySalesDispatchLinesShipFromInvoice: TWideStringField
      FieldName = 'ShipFromInvoice'
      FixedChar = True
      Size = 1
    end
  end
  object dsSalesDispatchLines: TDataSource
    DataSet = QrySalesDispatchLines
    Left = 67
    Top = 297
  end
  object cboEmployeeLookup: TERPQuery
    Connection = MyConnection
    SQL.Strings = (
      'SELECT tblEmployees.EmployeeID, EmployeeName  AS EmployeeName '
      'FROM tblEmployees '
      'WHERE (((tblEmployees.Active)<>'#39'F'#39')) '
      'ORDER BY EmployeeName;')
    AutoCalcFields = False
    Options.LongStrings = False
    Left = 107
    Top = 261
    object cboEmployeeLookupEmployeeID: TIntegerField
      FieldName = 'EmployeeID'
    end
    object cboEmployeeLookupEmployeeName: TWideStringField
      FieldName = 'EmployeeName'
      Size = 255
    end
  end
end
