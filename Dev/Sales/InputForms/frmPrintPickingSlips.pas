unit frmPrintPickingSlips;

interface

uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls,
  Dialogs, BaseInputForm, Grids, Wwdbigrd, Wwdbgrid, DB, StdCtrls,
  wwcheckbox, Buttons, DNMSpeedButton, ExtCtrls, DNMPanel, MemDS, DBAccess, MyAccess,ERPdbComponents,
  DataState, SelectionDialog, AppEvnts, BusObjPrintDoc,  Menus, AdvMenus, Shader,
  Forms, ImgList, ProgressDialog , JSONObject;

type
  TfmPrintPickingSlips = class(TBaseInputGUI)
    qrybins: TERPQuery;              
    pnlHeader: TDNMPanel;
  ...
  private
    ...
    fsSalesIdsToProcess: string;
    fPrintDoc :TBusObjPrintDoc;
  Protected
    function GetReportTypeID: integer; Override;
  public
    property SalesIdsToProcess: string read fsSalesIdsToProcess write fsSalesIdsToProcess;
//    Class Function SQL4PickingSlipreport(ReportName:String;var Json: TJsonObject; const TRansIDs:String = ''; TRansType:String=''; const fsBinfilter :STring=''; DetailSQL:String =''):String;
  end;

//Function PickingslipreporttoOrder(reportName:String):Integer;
implementation

uses FastFuncs,DNMLib, CommonLib, AppEnvironment,
  ProductQtyLib, busobjPQA,  tcConst, tcTypes, CommonDbLib, BusObjSales,
  BusobjCash, busobjPOS, EmailUtils, BusObjClient, saleslib, tcDataUtils,
  TemplateReportsLib, PrintPickingSlipsLib;

{$R *.dfm}

procedure TfmPrintPickingSlips.cmdPrintClick(Sender: TObject);
var
  sSql: string;
  i: integer;
  TemplateName : String;
...
begin
  ...
  try
    ...
    for I := 0 to grdMain.SelectedList.Count - 1 do begin
      qrybins.GotoBookmark(grdMain.SelectedList.Items[i]);
      if binfilter <> '' then binfilter := binfilter + ' OR ';
      if qrybinsBinLocation.AsString = 'Non-bin Products' then
          binfilter := binfilter + ' PBin.BinId = 0 OR isnull(PBin.binID) '
      else
          binfilter := binfilter + '( PBin.BinLocation = ' + QuotedStr(qrybinsBinLocation.AsString) +
                          ' and PBin.Binnumber = ' + QuotedStr(qrybinsBinnumber.AsString) +')';
    end;
    if binfilter = '' then Exit;
    binfilter := '(' + binfilter + ')';
      ...
      if chkChooseRpt.checked then
        if chkChooseRpt.Checked then begin
          ...
          if dlgReportSelect.Execute then
            TemplateName := dlgReportSelect.SelectedItems.Text;
        end;
      if TemplateName = '' then TemplateName := tcdatautils.GetDefaultReport(GetReportTypeID);
      if TemplateName = '' then TemplateName := tcdatautils.GetDefaultReport(30);
      if TemplateName = '' then TemplateName := 'Picking Slip';


      if salesIdsToProcess <> '' then begin
          if binfilter <> '' then binfilter := '(' + binfilter +') and ' ;
          binfilter := binfilter + ' (' + ReplaceStr(SalesIdstoProcess, ' SalesId = ',' S.SaleId = ') +')';
      end;

      SalesIdsToProcess := replacestr(SalesIdsToProcess , ' SalesId = ' , '');
      SalesIdsToProcess := replacestr(SalesIdsToProcess , ' OR' , ',');
      SalesIdsToProcess := replacestr(SalesIdsToProcess , '''' , '');

//      sSQL := TfmPrintPickingSlips.SQL4PickingSlipreport(TemplateName, json, SalesIdsToProcess, SalesTransTypes, binfilter);

      sSQL := PrintPickingSlipsLib.SQL4PickingSlipReport(TemplateName, json, SalesIdsToProcess, SalesTransTypes, binfilter);

      fbReportSQLSupplied := true;

      PrintTemplateReport(TemplateName, sSQL, False, 1 , json);

      fbReportSQLSupplied := false;
    if SalesIdsToProcess <> '' then
      with TEmpMyquery do try
        SQl.add('Select clientID, SaleId, IsInvoice,isquote,IsSalesOrder,IscashSale,IsPos from tblSales where saleId in (' +  SalesIdsToProcess+')');
        open;
        if recordcount >0 then begin
          ...
          Try
            While Eof = False do begin
                       if fieldbyname('IsInvoice').asBoolean    then begin
                        if fPrintDoc.PrintDoc(TInvoice.ClassName    , fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname ,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview) , TemplateName)<=0 then exit;
              end else if fieldbyname('IsSalesOrder').asBoolean then begin if fPrintDoc.PrintDoc(TSalesOrder.ClassName , fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname ,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview) , TemplateName)<=0 then exit;
              end else if fieldbyname('IsCashSale').asBoolean   then begin if fPrintDoc.PrintDoc(TCashSale.ClassName   , fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname ,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview) , TemplateName)<=0 then exit;
              end else if fieldbyname('isquote').asBoolean   then begin if fPrintDoc.PrintDoc(TQuote.ClassName   , fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname ,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview) , TemplateName)<=0 then exit;
              end else if fieldbyname('IsPOs').asBoolean   then begin if fPrintDoc.PrintDoc(TPos.ClassName   , fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname ,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview), TemplateName)<=0 then exit;
              end else begin if fPrintDoc.PrintDoc(TSales.Classname, fieldbyname('saleId').asInteger, dtPickSlip, Self.Classname,PreviewToDocumentOutputType(AppEnv.Employee.ShowPreview) , TemplateName )<=0 then exit;
              end;
...
procedure TfmPrintPickingSlips.DNMSpeedButton1Click(Sender: TObject);
begin
  inherited;
  opendb(qrybins);
  if qrybins.active = False then exit;
  grdmain.SelectAll;
end;

procedure TfmPrintPickingSlips.FormShow(Sender: TObject);
var
  strSql: string;
begin
  DisableForm;
  try
    ...
    strsql := ' Select distinct tblProductBin.Binlocation ,  tblProductBin.BinNumber ' +
                ' from tblPQA as PQA  ' +
                ' inner join tblPQADetails as PQADetails on PQADetails.PQAID = PQA.PQAID and PQADetails.PQAtype = ' + quotedstr(TPQABins.PQADetailtype)  +
                ' inner join tblProductBin on tblProductBin.binId = PQADetails.value ' +
                ' where PQA.TransType in (' + SalesTransTypes +')';
    if SalesIdsToProcess <> '' then
      strSql := strSql + ' and (' + replacestr(SalesIdsToProcess , 'SalesId' , 'PQA.TransId') +')';

    strSql := strSql + ' union' +
              '  Select distinct ''Non-bin Products'' , "" ' +
              '  from tblParts as P  ' +
              ' inner join tblSalesLines as SL on SL.ProductId = P.PartsID and P.Multiplebins = "F" ' +
              ' inner join tblsales as S on S.SaleId = SL.SaleId and (S.isInvoice = "T" or S.ISSalesOrder = "T")';

    if SalesIdsToProcess <> '' then
      strSql := strSql + ' Where ' + replacestr(SalesIdsToProcess , 'SalesId' , 'S.SaleID') ;

    if qrybins.Active then qrybins.Close;
    qrybins.SQL.Clear;
    qrybins.SQL.add(strSql);
    Qrybins.Open;
  finally
    EnableForm;
  end;   
...
function TfmPrintPickingSlips.GetReportTypeID: integer;
begin
  Result := 85;
end;

procedure TfmPrintPickingSlips.FormClose(Sender: TObject;
...
procedure TfmPrintPickingSlips.FormCreate(Sender: TObject);
begin
  inherited;
...
{}
procedure TfmPrintPickingSlips.btnCloseClick(Sender: TObject);
begin
  inherited;
  Self.Close;
end;
...
//class function TfmPrintPickingSlips.SQL4PickingSlipreport(REportName:String; var Json: TJsonObject; const TRansIDs:String = '';  TRansType:String=''; const fsBinfilter :STring=''; DetailSQL:String =''): String;
//var
//  sSQL:String;
//  PqaString :STring;
...
//
//  PqaString := 'and ((P.multiplebins = "F" and P.Batch="F" and P.SNTracking = "F") OR (PQA.PQAID in (Select PQAID from tblPQADetails)))';
//
//  if Transtype = '' then Transtype := SalesTransTypes else if Transtype <> SalesTransTypes then Transtype := QuotedStr(Transtype);
//  {inventory items}
//  sSQL := '{Detail}SELECT  ' +
//        'SL.Product_Description as PARTSDESCRIPTION,' +
...
//        'SL.Product_Description_Memo as PARTSDESCRIPTION_Memo,' +
//        'SL.RequiresAssembly,'+
//        'SL.Attrib1Sale,' +
//        'SL.Attrib2Sale,' +
//        'SL.MemoLine ,'+
...
//        SplitField('PQABatch', 'Value', 1, '-')+' as BatchGroup,'+
//        SplitField('PQABatch', 'Value', 2, '-')+' as BatchDetails,'+
//        'PQABatch.TruckLoadNo , '+
//        'PQABatch.ExpiryDate, '+
//        'PQABatch.CUSTFLD1 as BatchCustFld1, '+
//        'Round(sum(if(ifnull(PQASN.PQADEtailID,0)<>0 , PQASN.UOMQty ,if (ifnull(PQABins.PQADEtailID,0)<>0 , PQABins.UOMQty , if(ifnull(PQABatch.UOMQty,0)<>0 , PQABatch.uomQty , PQA.uomQty)))), ' +inttostr(tcConst.GeneralRoundPlaces) +') as BinQty,'+
...
//          'round(if(P.SNtracking  ="T" or P.MultipleBins="T" or P.Batch="T" , "Qty Allocated" , ""), ' +inttostr(tcConst.GeneralRoundPlaces) +')  as QtyAllocated, '+
//          'Round(if(P.SNtracking  ="T" or P.MultipleBins="T" or P.Batch="T" , "Qty Picked" , ""), ' +inttostr(tcConst.GeneralRoundPlaces) +')  as QtyPicked, '+
...
//        ' if (HoldSale = "T", "HOLD", "") AS Hold, ' + CHR(13) +
//        ' P.BARCODE as ProductBarCode,'+ CHR(13) +
//        ' CONCAT(SL.ProductName,Product_Description) AS GroupLine, ' + CHR(13) +
...
//        ' CL.Mobile as CustomerMobile,   ' + CHR(13) +
//        Thirdcolumn  + '  as Dept, ' + CHR(13) +
//        SecondColumn + ' as type, ' + CHR(13) +
//        Firstcolumn  + ' as Manufacture, ' ;
//
//    //if (PickingSlipOrder = 5) or (PickingSlipOrder = 0) then
//      sSQL :=sSQL + 'Sl.SaleLineID ,'+
//        'sum(SL.UnitofMeasureShipped)   as qShipped, '+
...
//        'sum(SL.LinePriceInc) as LinePriceInc, '+
//        'sum(SL.LinePrice) as LinePriceEx, ';
//
//         if PickingSlipOrder = 0 then sSQL := sSQL + 'CONCAT( "Product Details :" , SL.ProductName,Product_Description) AS Groupfield '
//    else if PickingSlipOrder = 1 then sSQL := sSQL + 'CONCAT( "Bin Location / Number:" , PBin.BinLocation,"-",PBin.BinNumber) as GroupField'
//    else if PickingSlipOrder = 2 then sSQL := sSQL + 'CONCAT('+QuotedStr(AppEnv.DefaultClass.FirstColumn) +',' + quotedStr(': ')+',' + firstcolumn  +')  as GroupField'
//    else if PickingSlipOrder = 3 then sSQL := sSQL + 'CONCAT('+ QuotedStr(AppEnv.DefaultClass.SecondColumn)+',' + quotedStr(': ')+',' + Secondcolumn +')  as GroupField'
//    else if PickingSlipOrder = 4 then sSQL := sSQL + 'CONCAT('+ QuotedStr(AppEnv.DefaultClass.ThirdColumn) +',' + quotedStr(': ')+',' + thirdcolumn  +')  as GroupField'
//    else if PickingSlipOrder = 5 then sSQL := sSQL + 'Concat('+ QuotedStr('Invoice No: ') + ', S.invoicedocnumber) AS Groupfield ' ;
//
//    sSQL := sSQL + ',CLT.TypeName,' +
//         'DPB.BinLocation as DefaultBinLocation,DPB.BinNumber as DefaultBinNumber ';
//    sSQL := sSQL + ',P.CUSTFLD1 AS ProductCustFld1, '+
...
//    sSQL := sSQL +chr(13) +' FROM ' + StringReplace(ProductTables(tDetailswithSno, False ,False , 0 , False ), PqaString ,'' , [rfIgnoreCase]) +
...
//
//    if fsBinfilter <> '' then sSQL := sSQL +' AND ' + fsBinfilter;
//
//    sSQL := sSQL +chr(13) +' Group by GroupField,   ';
//    if PickingSlipOrder = 5 then
//        sSQl := sSQl +'S.SaleID, SL.SortID,Sl.SalelineID,  ';
//    sSQL := sSQL +' PBin.binnumber,PBin.Binlocation, PQABatch.Value , PQABatch.ExpiryDate, ProductID, SL.SaleLineID, PQASN.value';
//
//
//  {non inventory items}
...
//        'SL.Product_Description_Memo as PARTSDESCRIPTION_Memo,' +
//        'SL.RequiresAssembly,'+
//        'SL.Attrib1Sale,SL.Attrib2Sale,SL.MemoLine ,' +
//        'Null as Binlocation,'+
...
//        'null as TruckLoadNo,'+
//        'null as ExpiryDate,'+
//        'null as BatchCustFld1,'+
//        'null as BinQty,'+
//        'null as SoldSerials, '+
//        'null as BatchnumberCaption,'+
//        'null as ExpiryDateCaption ,'+
//        'null as BinLocationCaption,'+
//        'null as BinnumberCaption,'+
//        'null as SerialnumberCaption, '+
//        'null as QtyAllocated, '+
//        'null as QtyPicked,  '+
//        'CONCAT("S-",S.SaleId) as SaleBarcode, s.*,'+
//        'SL.SalesLinesCustField1,SL.SalesLinesCustField2,SL.SalesLinesCustField3,SL.SalesLinesCustField4,SL.SalesLinesCustField5,SL.SalesLinesCustField6,SL.SalesLinesCustField7,SL.SalesLinesCustField8,SL.SalesLinesCustField9,SL.SalesLinesCustField10,'+
//        '`SL`.`WarrantyPeriod`,`SL`.`WarrantyEndsOn`, SL.SortId,'+
...
//
//         if PickingSlipOrder = 0 then sSQL := sSQL + 'CONCAT("Product Details :" , SL.ProductName,Product_Description) AS Groupfield '
//    else if PickingSlipOrder = 1 then sSQL := sSQL + 'null  as GroupField'
//    else if PickingSlipOrder = 2 then sSQL := sSQL + 'CONCAT(' + QuotedStr(AppEnv.DefaultClass.FirstColumn) +',' +quotedStr(': ')+', ' + firstcolumn  +')  as GroupField'
//    else if PickingSlipOrder = 3 then sSQL := sSQL + 'CONCAT(' + QuotedStr(AppEnv.DefaultClass.SecondColumn)+',' +quotedStr(': ')+', ' + Secondcolumn +')  as GroupField'
//    else if PickingSlipOrder = 4 then sSQL := sSQL + 'CONCAT(' + QuotedStr(AppEnv.DefaultClass.ThirdColumn) +',' +quotedStr(': ')+', ' + thirdcolumn  +')  as GroupField'
//    else if PickingSlipOrder = 5 then sSQL := sSQL + 'Concat(' + QuotedStr('Invoice No: ') + ', S.invoicedocnumber) AS Groupfield ' ;
...
//    end else if PickingSlipOrder = 1 then begin // bins
//      sSQL := sSQL + ' ORDER BY  Groupfield, BatchGroup, BatchDetails, batchno ;';
//    end else if PickingSlipOrder = 2 then begin // first columne (Manufacture)
//      sSQL := sSQL + ' ORDER BY Groupfield, Manufacture';//Product_Description';
//    end else if PickingSlipOrder = 3 then begin // second column
//      sSQL := sSQL + ' ORDER BY Groupfield,type';//Product_Description';
//    end else if PickingSlipOrder = 4 then begin // third column
//      sSQL := sSQL + ' ORDER BY Groupfield,Dept'; //Product_Description';
//    end else if PickingSlipOrder = 5 then begin // defualt order
...
//  if DetailSQL <> '' then sSQL := sSQL +chr(13) +'~|||~' + DetailSQL;
//
//  sSQL := sSQL + '~|||~{Master}SELECT  CompanyName, Address, Address2, City, State, Postcode,  ' +
//        ' PhoneNumber AS PhoneNumber,FaxNumber AS FaxNumber, ABN,' +
//        ' Email as CompanyEmail, URL as CompanyURL' +
//        ' FROM tblCompanyInformation ' ;
//  sSQL := sSQL + '~|||~{AttribCaptions}SELECT  ' +
...
//          QuotedStr('Price / ' + AppEnv.CompanyPrefs.ProductAttrib1Name)+ ' as ProductAttrib1SaleRate';
//
...
//        'from ' +
//        'tblparts sP ' +
//        'inner join tblpqa sPQA  on sPQA.productId = sP.partsId ' +
...
//
//        'Round(sum(if (ifnull(PQABins.PQADEtailID,0)<>0 , PQABins.UOMQty , PQABatch.UOMQty)) , ' +inttostr(tcConst.GeneralRoundPlaces) +') as allocatedqty,'+
//        'Round(sum(if (ifnull(PQABins.PQADEtailID,0)<>0 , PQABins.BOUOMQty , PQABatch.BOUOMQty)) , ' +inttostr(tcConst.GeneralRoundPlaces) +') as BOallocatedqty,'+
//        'Round(sum(PQABatch.CUSTFLD1) , ' +inttostr(tcConst.GeneralRoundPlaces) +') as allocatedLength,'+
//
...
//                   'P.CUSTDATE3 AS ProductCustDate3 ';
//
// sSQL := sSQL + ' FROM  tblSaleslines SL ' +
//                ' inner JOIN tblsales AS S ON  SL.SaleID = S.SaleID ' +
//                ' inner join tblClients CL on CL.ClientID = S.ClientID ' +
//                ' Left join tblclienttype CLT on CLT.ClientTypeID = CL.ClientTypeID ' +
//                ' inner join `tblParts` as P on SL.ProductId = P.PartsId ' +
//                ' inner join `tblPQA` as PQA on PQA.transId = SL.saleId and PQA.translineId =  Sl.salelineId and PQA.transtype in ('+ SalesTransTypes +' ) ' +
...
//      sSQL := sSQL +' AND  S.SaleID in( ' + TRansIDs +')'; //+' and PQA.transtype in (' + Transtype+')'
//
//    if fsBinfilter <> '' then sSQL := sSQL +' AND ' + fsBinfilter;
//
...
//    if PickingSlipOrder = 5 then
//        sSQl := sSQl +'S.SaleID, SL.SortID,Sl.SalelineID,  ';
//    sSQL := sSQL +' PBin.binnumber,PBin.Binlocation, PQABatch.Value , PQABatch.ExpiryDate, ProductID, SL.SaleLineID ' +
//      'having ';
...
//      sSQL :=  sSQL +' Order by Classname, BatchGroup, BatchDetails, batchno ,TruckLoadNo ,  binlocation, binnumber, uom '
//    else if Sametext(Reportname ,  'Picking Slip with Batch Qty with Totals' ) then
//      sSQL :=  sSQL +' Order by Classname,Productname, BatchGroup, BatchDetails, batchno ,TruckLoadNo ,  binlocation, binnumber, uom '
//    else
...
//              ' Where TTC.SaleId in( ' + TRansIDs +') group by TTCL.`SubTaxCode` ';
...
//
//    sSQL:= sSQL+'~|||~{RawMaterials}select  '+
//              ' sl.saleId, sl.salelineId,  '+
...
//              ' where sl.saleID in( ' + TRansIDs +')  and (ifnull(ppt.ProcTreeId,0)=0 or ppt.InputType <> "itOption" or (ppt.InputType = "itOption" and pt.Selected ="T"))';
//
...
//          (Sysutils.SameText(REportName ,'Pick Slip(Product)'))          then result := 0
//  else if (SameText(copy(REportName ,1, Length('Picking Slip List(Bin)')) , 'Picking Slip List(Bin)')) or
//          (SameText(copy(REportName ,1, Length('Pick slip(Bin)')) , 'Pick slip(Bin)')) then result := 1
//  else if (Sysutils.SameText(REportName ,'Picking Slip List(Col1)') )    or
//          (Sysutils.SameText(REportName ,'Pick Slip(Col1)'))             then result := 2
//  else if (Sysutils.SameText(REportName ,'Picking Slip List(col2)') )    or
//          (Sysutils.SameText(REportName ,'Pick Slip(Col2)'))             then result := 3
//  else if (Sysutils.SameText(REportName ,'Picking Slip List(Col3)') )    or
//          (Sysutils.SameText(REportName ,'Pick Slip(Col3)'))             then result := 4
//  else if (Sysutils.SameText(REportName ,'Picking Slip List') )          or
//          (Sysutils.SameText(REportName ,'Picking Slip'))                then result := 5
//  else result := 5;
//end;

initialization
  RegisterClassOnce(TfmPrintPickingSlips);
