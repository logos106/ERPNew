...
//(It just includes sending my kids a postcard, nothing more !)
  ...
  //Fixed : Pete M
  //Change: In some (odd) circumstances FastMemposex(NC) would return a true result
...
type
  ...
  TFastPosIndexProc = function(const aSourceString, aFindString: string; const aSourceLen, aFindLen, StartPos: integer;
...
function BMPosNoCase(const aSource, aFind: Pointer; const aSourceLen, aFindLen: integer;
  var JumpTable: TBMJumpTable): Pointer;
...
function FastTagReplace(const SourceString, TagStart, TagEnd: string; FastTagReplaceProc: TFastTagReplaceProc;
  const UserData: integer): string;
function Smartposex(const SearchStr, SourceStr: string; const CaseSensitive: boolean = true;
...
@@TheEnd:
  ...
  //       they are used by FastReplace many times over, thus saving a char_length()
...
begin
  ...
  Result := integer(BMPos(PChar(aSourceString) + (StartPos - 1), PChar(aFindString),
    aSourceLen - (StartPos - 1), aFindLen, JumpTable));
  if Result > 0 then Result := Result - integer(@aSourceString[1]) + 1;
end;
...
begin
  ...
  Result := integer(BMPosNoCase(PChar(aSourceString) + (StartPos - 1), PChar(aFindString),
    aSourceLen - (StartPos - 1), aFindLen, JumpTable));
  if Result > 0 then Result := Result - integer(@aSourceString[1]) + 1;
end;
...
begin
  ...
  if (StartPos = 0) or (StartPos + aFindLen > aSourceLen) then SourceLen := aSourceLen - (aFindLen - 1)
  else SourceLen := StartPos;
...
begin
  ...
  if (SourceLength = 0) or (FindLength = 0) or
    (FindLength > SourceLength) then begin
    Result := S;
    Exit;
  end;
  ...
  if rfIgnoreCase in Flags then begin
    ...
    FindString   := uppercase(OldPattern);
  end else begin
    SourceString := S;
  ...
  try
    if ReplaceLength <= FindLength then //Result cannot be larger, only same size or smaller
      BufferSize := SourceLength
    else //Assume a source string made entired of the sub string
    ...
    if BufferSize > (SourceLength * 10) then BufferSize := SourceLength * 10;
  except
  ...
  if ReplaceLength > 0 then ReplacePChar := @NewPattern[1]
  else ReplacePChar := nil;

  //Replace routine
  repeat
    //Find the sub string
    SourceStringPChar := AnsiStrPos(PrevSourceStringPChar,FindPChar);
    ...
    if CopySize + ReplaceLength > BufferSize - BytesWritten then begin
    ...
    if Assigned(ReplacePChar) then begin
  ...
  until (PrevSourceStringPChar > FinalSourceMarker) or not (rfReplaceAll in Flags);

  FinalSourcePosition := integer(SPChar - @S[1]);
  CopySize := SourceLength - FinalSourcePosition;
  Setlength(Result, BytesWritten + CopySize);
  if CopySize > 0 then Move(SPChar^, Result[BytesWritten + 1], CopySize);
end;
...
begin
  ...
  try
    if LReplace <= LFind then Setlength(Result, lSource)
    else Setlength(Result, (LSource * LReplace) div LFind);
  except
    Setlength(Result, 0);
  end;
  ...
  if LSource > 0 then FastCharMove(PSource^, Result[BytesUsed + 1], LSource);
end;
...
var
  ...
  begin
    if BytesWritten + Size > BufferSize then begin
      BufferSize := Trunc(BufferSize * cDeltaSize);
      if BufferSize <= (BytesWritten + Size) then BufferSize := Trunc((BytesWritten + Size) * cDeltaSize);
      Setlength(Result, BufferSize);
...
begin
  ...
  repeat
    ...
    if CopySize > 0 then Move((TagStartFindPos + TagStartLength)^, ReplaceString[1], CopySize);
    FastTagReplaceProc(ReplaceString, UserData);
    if Length(ReplaceString) > 0 then AddBuffer(@ReplaceString[1], Length(ReplaceString));
  ...
  if CopySize > 0 then AddBuffer(SourceStringPChar, CopySize);
...
begin
  ...
  if (CaseSensitive) then if (ForwardSearch) then Result :=
  ...
  else if (ForwardSearch) then Result :=
