...
function ExtractHTML(const S: string): string;
function ExtractNonHTML(const S: string): string;
//function HexToInt(const aHex: string): int64;
function LeftStr(const aSourceString: string; Size: integer): string;
//function StringMatches(Value, Pattern: string): boolean;
function MissingText(const Pattern, Source: string; const SearchText: string = '?'): string;
function RandomFileName(const aFilename: string): string;
...
function UniqueFilename(const aFilename: string): string;
//function URLToText(const aValue: string): string;
function WordAt(const Text: string; const Position: integer): string;

procedure Split(const aValue: string; const aDelimiter: char; var Result: TStringList);

implementation

uses FastStrings, FastFuncs;
...
begin
  ...
  while Position > 0 do begin
    ...
    if Value = -1 then raise Exception.Create('Invalid hex character ' + aHex[Position]);
...
function LeftStr(const aSourceString: string; Size: integer): string;
begin
  if (Size < 0) or (Size > Length(aSourceString)) then Result := aSourceString
  else begin
...
begin
  ...
  if Star1 = 0 then Result := (Value = Pattern)
  else begin
    ...
    if Result then begin
      ...
      //posex(NextPattern,Value);
      NextPos := FastPos(Value, NextPattern, Length(Value), Length(NextPattern), 1);
      if (NextPos = 0) and not (NextPattern = '') then Result := false
      else begin
        Value := Copy(Value, NextPos, Length(Value));
        if Pattern = '' then Result := true
        else Result := Result and StringMatches(Value, Pattern);
      end;
    end;
  end;
end;
...
var
  ...
  lSearchText, lBeforeText, lAfterText, lSource: longint;
begin
  Result := '';
  Position := PosEx(SearchText, Pattern);
...
begin
  ...
  if Length(Ext) > 0 then FileName := Copy(FileName, 1, Length(FileName) - Length(Ext));
  repeat
...
var
  ...
  Multiple: word;
begin
  if aSourceString = '' then Result := 0
  else begin
    ...
    for I := 1 to Length(aSourceString) do begin
      Inc(CurrentChar);

      S := Ord(CurrentChar^);
      if (S > 64) and (S < 123) then begin
        SoundexGroup := cSoundexTable[S];
        if (SoundexGroup <> LastChar) and (SoundexGroup > 0) then begin
  ...
  for X := 1 to Length(S) do begin
    case SrcChar^ of
      '<':
        begin
          Inc(TagCnt);
          if WantHTML and (TagCnt = 1) then begin
            ResChar^ := '<';
            Inc(ResChar);
          end;
        end;
      '>':
        begin
          Dec(TagCnt);
          if WantHTML and (TagCnt = 0) then begin
            ResChar^ := '>';
            Inc(ResChar);
          end;
        end;
      else case WantHTML of
          false: if TagCnt <= 0 then begin
              ResChar^ := SrcChar^;
              Inc(ResChar);
              TagCnt := 0;
            end;
          true: if TagCnt >= 1 then begin
              ResChar^ := SrcChar^;
              Inc(ResChar);
            end else if TagCnt < 0 then TagCnt := 0;
        end;
    end;
    Inc(SrcChar);
  end;
...
begin
  ...
  while X <= Length(aValue) do begin
    if aValue[X] <> '%' then Result := Result + aValue[X]
    else begin
...
begin
  ...
  for X := Position to L do begin
    if CharInSet(Text[X] , ['A'..'Z', '0'..'9']) then Result := Result + Text[X]
    else Break;
  end;

  for X := Position - 1 downto 1 do begin
    if CharInSet(Text[X] , ['A'..'Z', '0'..'9']) then Result := Text[X] + Result
    else Break;
  end;
end;
