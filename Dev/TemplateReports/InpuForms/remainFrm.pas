...

{
 ...
 -------- ------- ---  ------------------------------------------------------
 26/08/05 1.00.01 DSP  Set fbIgnoreAccessLevels to True.
...
uses
  ...
  ToolWin, ActnList, ImgList, DB,  BaseInputForm, Mask, DBCtrls,
  MyAccess,ERPdbComponents, MemDS, DBAccess, DataState, SelectionDialog, AppEvnts,
  ProgressDialog;

type
  TReMain = class(TBaseInputGUI)
    MainMenu: TMainMenu;
  ...
  public
    ...
    property EmpID: integer read fEmpID write SetEmpID;
  end;
...
procedure TReMain.SelectionChange(Sender: TObject);
begin
...
function TReMain.CurrText: TTextAttributes;
begin
...
procedure TReMain.GetFontNames;
var
  DC: HDC;
begin
  DC := GetDC(0);
...
procedure TReMain.SetFileName(const FileName: string);
begin
  FFileName := FileName;
  Caption := Format('%s - %s', [ExtractFileName(FileName), Application.Title]);
end;

procedure TReMain.CheckFileSave;
var
  SaveResp: integer;
begin
...
procedure TReMain.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TReMain.FileNew(Sender: TObject);
begin
  SetFileName(sUntitled);
  Editor.Lines.Clear;
...
procedure TReMain.PerformFileOpen(const AFileName: string);
begin
...
begin
  ...
  with Editor do begin
    ...
    ADOTable1.Filter   := 'EmployeeID=' + QuotedStr(IntToStr(EmpID));
    ...
    if Fax.Text = '' then begin
      ...
      txtLine := '';
      if ADOTable1.FieldByName('Title').AsString <> '' then txtLine := ADOTable1.FieldByName('Title').AsString + ' ';
      if ADOTable1.FieldByName('FirstName').AsString <> '' then txtLine := txtLine + ADOTable1.FieldByName('FirstName').AsString + ' ';
      if ADOTable1.FieldByName('LastName').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('LastName').AsString;
      Lines.Add(txtLine);
      {Contact Company}
      txtLine := '';
      if ADOTable1.FieldByName('Company').AsString <> '' then txtLine := ADOTable1.FieldByName('Company').AsString;
      Lines.Add(txtLine);
      {Contact Address}
      txtLine := '';
      if ADOTable1.FieldByName('Street').AsString <> '' then txtLine := ADOTable1.FieldByName('Street').AsString + ' ';
      if ADOTable1.FieldByName('Street2').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('Street2').AsString;
      Lines.Add(txtLine);
      {Contact Suburb/State/Postcode}
      txtLine := '';
      if ADOTable1.FieldByName('Suburb').AsString <> '' then txtLine := ADOTable1.FieldByName('Suburb').AsString + ' ';
      if ADOTable1.FieldByName('State').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('State').AsString + ' ';
      if ADOTable1.FieldByName('Postcode').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('Postcode').AsString;
      Lines.Add(txtLine);
      {Two returns}
      Lines.Add('');
      Lines.Add('');
      {Salutation}
      txtLine := '';
      if ADOTable1.FieldByName('FirstName').AsString <> '' then txtLine := 'Dear ' + ADOTable1.FieldByName('FirstName').AsString;
      Lines.Add(txtLine);
      {Close Contact Table}
      ADOTable1.Close;
      {One returns}
      ...
      Paragraph.Alignment := taLeftJustify;
      txtLine := 'To: ';
      if ADOTable1.FieldByName('Title').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('Title').AsString + ' ';
      if ADOTable1.FieldByName('FirstName').AsString <> '' then txtLine := txtLine + ADOTable1.FieldByName('FirstName').AsString + ' ';
      if ADOTable1.FieldByName('LastName').AsString <> '' then txtLine :=
          txtLine + ADOTable1.FieldByName('LastName').AsString;
      txtSave := txtLine;
      if ADOTable1.FieldByName('FaxNumber').AsString <> '' then txtLine := txtLine + '                              Fax No: ' + ADOTable1.FieldByName('FaxNumber').AsString
...
procedure TReMain.FileOpen(Sender: TObject);
begin
  CheckFileSave;
...
procedure TReMain.FileSave(Sender: TObject);
begin
...
procedure TReMain.FileSaveAs(Sender: TObject);
begin
...
procedure TReMain.FilePrint(Sender: TObject);
begin
  if PrintDialog.Execute then Editor.Print(FFileName);
end;

procedure TReMain.FileExit(Sender: TObject);
begin
  Close;
end;

procedure TReMain.EditUndo(Sender: TObject);
begin
  with Editor do if HandleAllocated then SendMessage(Handle, EM_UNDO, 0, 0);
end;

procedure TReMain.EditCut(Sender: TObject);
begin
  Editor.CutToClipboard;
end;

procedure TReMain.EditCopy(Sender: TObject);
begin
  Editor.CopyToClipboard;
end;

procedure TReMain.EditPaste(Sender: TObject);
begin
  Editor.PasteFromClipboard;
end;

procedure TReMain.SelectFont(Sender: TObject);
begin
...
procedure TReMain.RulerResize(Sender: TObject);
begin
  RulerLine.Width := Ruler.ClientWidth - (RulerLine.Left * 2);
end;

procedure TReMain.FormResize(Sender: TObject);
begin
  SetEditRect;
  SelectionChange(Sender);
end;

procedure TReMain.FormPaint(Sender: TObject);
begin
  SetEditRect;
end;

procedure TReMain.BoldButtonClick(Sender: TObject);
begin
...
procedure TReMain.ItalicButtonClick(Sender: TObject);
begin
  if FUpdating then Exit;
...
procedure TReMain.FontSizeChange(Sender: TObject);
begin
  if FUpdating then Exit;
  CurrText.Size := FastFuncs.StrToInt(FontSize.Text);
end;

procedure TReMain.AlignButtonClick(Sender: TObject);
begin
  if FUpdating then Exit;
  Editor.Paragraph.Alignment := TAlignment(TControl(Sender).Tag);
end;

procedure TReMain.FontNameChange(Sender: TObject);
begin
  if FUpdating then Exit;
  CurrText.Name := FontName.Items[FontName.ItemIndex];
end;

procedure TReMain.UnderlineButtonClick(Sender: TObject);
begin
  if FUpdating then Exit;
...
procedure TReMain.BulletsButtonClick(Sender: TObject);
begin
  if FUpdating then Exit;
  Editor.Paragraph.Numbering := TNumberingStyle(BulletsButton.Down);
end;

procedure TReMain.FormCloseQuery(Sender: TObject; var CanClose: boolean);
begin
  try
    CheckFileSave;
  except
...
procedure TReMain.RulerItemMouseDown(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
...
procedure TReMain.RulerItemMouseMove(Sender: TObject; Shift: TShiftState; X, Y: integer);
begin
  if FDragging then TLabel(Sender).Left := TLabel(Sender).Left + X - FDragOfs
end;

procedure TReMain.FirstIndMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
  FDragging := false;
...
procedure TReMain.LeftIndMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
  FDragging := false;
...
procedure TReMain.RightIndMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: integer);
begin
  FDragging := false;
...
procedure TReMain.UpdateCursorPos;
var
  CharPos: TPoint;
begin
...
procedure TReMain.FormShow(Sender: TObject);
var
  FileName: string;
begin
  DisableForm;
...
procedure TReMain.WMDropFiles(var Msg: TWMDropFiles);
var
...
procedure TReMain.RichEditChange(Sender: TObject);
begin
  SetModified(Editor.Modified);
end;

procedure TReMain.SetModified(Value: boolean);
begin
  if Value then StatusBar.Panels[1].Text := sModified
  else StatusBar.Panels[1].Text          := '';
end;

procedure TReMain.SwitchLanguage(Sender: TObject);
var
  Name: string;
  Size: integer;
begin
...
procedure TReMain.ActionList2Update(Action: TBasicAction; var Handled: boolean);
begin
...
procedure TReMain.FileEmail(Sender: TObject);
var
  strMsg: string;
  I: integer;
begin
...
procedure TReMain.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure TReMain.SetEmpID(const Value: integer);
begin
  fEmpID := Value;
end;

initialization
  RegisterClassOnce(TReMain);
end.
