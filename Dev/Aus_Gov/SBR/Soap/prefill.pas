...
// WSDL     : C:\Dev\ERP\Dev\Aus_Gov\SBR\DotNet\C_Sharp\SBR_CoreServicesRequester_C#_1.2.0_2015-06-09\SBR_CoreServicesRequester\WSDL\Core\prefill.02.01.service.wsdl
//  >Import : C:\Dev\ERP\Dev\Aus_Gov\SBR\DotNet\C_Sharp\SBR_CoreServicesRequester_C#_1.2.0_2015-06-09\SBR_CoreServicesRequester\WSDL\Core\prefill.02.01.service.wsdl>0
...
// Encoding : UTF-8
// Version  : 1.0
// (19/06/2018 9:29:39 AM - - $Rev: 34800 $)
// ************************************************************************ //
...
type
  ...
  ResponsePreFillReport = class;                { "http://sbr.gov.au/comn/prefill.02.service"[Lit][GblElm] }
  RequestPreFillReport = class;                 { "http://sbr.gov.au/comn/prefill.02.service"[Lit][GblElm] }
  ...
  // ************************************************************************ //
  // XML       : ResponsePreFillReport, global, <element>
  // Namespace : http://sbr.gov.au/comn/prefill.02.service
  // Serializtn: [xoLiteralParam]
  // Info      : Wrapper
  // ************************************************************************ //
  ResponsePreFillReport = class(TRemotable)
  private
  ...
  // ************************************************************************ //
  // XML       : RequestPreFillReport, global, <element>
  // Namespace : http://sbr.gov.au/comn/prefill.02.service
  // Serializtn: [xoLiteralParam]
  // Info      : Wrapper
  // ************************************************************************ //
  RequestPreFillReport = class(TRemotable)
  private
    FStandardBusinessDocumentMessage: StandardBusinessDocumentMessage;
  public
    constructor Create; override;
  ...
  // ************************************************************************ //
  // Namespace : http://sbr.gov.au/comn/prefill.02.service
  // soapAction: http://sbr.gov.au/services/prefill.02.service/PreFillReport
  // transport : http://schemas.xmlsoap.org/soap/http
  // style     : document
  // use       : literal
  // binding   : PreFill.02.SoapBinding
  // service   : PreFill.02.Service
  // port      : PreFill.02.Port
  // URL       : https://sbr.gov.au/services/prefill.02.service
  // ************************************************************************ //
  IPreFill02 = interface(IInvokable)
  ['{7DEE5B83-E9E5-E659-07EB-C3FA9A2B1E79}']

    // Cannot unwrap: 
    //     - Input element wrapper name does not match operation's name
    function  PreFillReport(const parameters: RequestPreFillReport): ResponsePreFillReport; stdcall;
  end;

function GetIPreFill02(UseWSDL: Boolean=System.False; Addr: string=''; HTTPRIO: THTTPRIO = nil): IPreFill02;


implementation
  uses SysUtils;

function GetIPreFill02(UseWSDL: Boolean; Addr: string; HTTPRIO: THTTPRIO): IPreFill02;
const
  defWSDL = 'C:\Dev\ERP\Dev\Aus_Gov\SBR\DotNet\C_Sharp\SBR_CoreServicesRequester_C#_1.2.0_2015-06-09\SBR_CoreServicesRequester\WSDL\Core\prefill.02.01.service.wsdl';
  defURL  = 'https://sbr.gov.au/services/prefill.02.service';
  defSvc  = 'PreFill.02.Service';
  defPrt  = 'PreFill.02.Port';
var
  RIO: THTTPRIO;
begin
  Result := nil;
...
constructor ResponsePreFillReport.Create;
begin
  inherited Create;
  FSerializationOptions := [xoLiteralParam];
end;

destructor ResponsePreFillReport.Destroy;
begin
  SysUtils.FreeAndNil(FStandardBusinessDocumentMessage);
  inherited Destroy;
end;

constructor RequestPreFillReport.Create;
begin
  inherited Create;
  FSerializationOptions := [xoLiteralParam];
end;

destructor RequestPreFillReport.Destroy;
begin
  SysUtils.FreeAndNil(FStandardBusinessDocumentMessage);
  inherited Destroy;
end;
...
initialization
  { IPreFill02 }
  InvRegistry.RegisterInterface(TypeInfo(IPreFill02), 'http://sbr.gov.au/comn/prefill.02.service', 'UTF-8');
  InvRegistry.RegisterDefaultSOAPAction(TypeInfo(IPreFill02), 'http://sbr.gov.au/services/prefill.02.service/PreFillReport');
  InvRegistry.RegisterInvokeOptions(TypeInfo(IPreFill02), ioDocument);
  InvRegistry.RegisterInvokeOptions(TypeInfo(IPreFill02), ioLiteral);
  InvRegistry.RegisterInvokeOptions(TypeInfo(IPreFill02), ioSOAP12);
  ...
  RemClassRegistry.RegisterXSClass(ResponsePreFillReport, 'http://sbr.gov.au/comn/prefill.02.service', 'ResponsePreFillReport');
  RemClassRegistry.RegisterExternalPropName(TypeInfo(ResponsePreFillReport), 'StandardBusinessDocumentMessage', '[Namespace="http://sbr.gov.au/comn/sbdm.02.data"]');
  RemClassRegistry.RegisterSerializeOptions(ResponsePreFillReport, [xoLiteralParam]);
  RemClassRegistry.RegisterXSClass(RequestPreFillReport, 'http://sbr.gov.au/comn/prefill.02.service', 'RequestPreFillReport');
  RemClassRegistry.RegisterExternalPropName(TypeInfo(RequestPreFillReport), 'StandardBusinessDocumentMessage', '[Namespace="http://sbr.gov.au/comn/sbdm.02.data"]');
  RemClassRegistry.RegisterSerializeOptions(RequestPreFillReport, [xoLiteralParam]);
