...
uses
  ...
  FileAssociationDetails, ERPDbLookupCombo ;

type
  TfrmSupplier = class(TBaseInputGUI)
...
uses FastFuncs,CommonDbLib, frmCustomFieldsFrm, FormFactory, DnMLib, AuditObj,
  ...
  Commonlib, frmClientsOtherPhonesGUI,  MaskUtils,
   AppEnvironment, tcDataUtils,tcTypes , CommonFormLib, BusObjClient,
  ...
  SystemLib, IdAttachment, BusObjBase, EmailConst,
  frmSupplierCorrespondenceCreate, {$WARNINGS OFF}FileCtrl{$WARNINGS ON}, docPathLib,
  HTTPConst, UserCustomisedNames, MessageConst;

{$R *.DFM}

procedure TfrmSupplier.cmdOkClick(Sender: TObject);
begin
  DisableForm;
  try
...
procedure TfrmSupplier.cmdCancelClick(Sender: TObject);
begin
  DisableForm;
  try
...
procedure TfrmSupplier.PreSaveSupplier;
begin
  try
...
class procedure TfrmSupplier.CloseMe;
begin
  if FormStillOpen('TfrmSupplier') then begin
      TfrmSupplier(FindExistingComponent('TfrmSupplier')).Close;
    Application.ProcessMessages;
  end;
end;

procedure TfrmSupplier.cboTermsEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.cboShippingEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.cboskillsCloseUp(Sender: TObject; LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  inherited;
...
procedure TfrmSupplier.cmdNotesClick(Sender: TObject);
begin
  inherited;
  TabCtl20.ActivePage := TTabSheet(Supplier_Info);
...
procedure TfrmSupplier.BillSuburbCloseUp(Sender: TObject; LookupTable, FillTable: TDataSet; Modified: boolean);
begin
...
function TfrmSupplier.CustomFieldExtenderListtype: TListType;
begin
  Result := ltSupplier;
end;

procedure TfrmSupplier.CustomFieldsRefresh;
begin
  CustomFieldRefresh('CFSupp' ,pnlCustFields);
end;
procedure TfrmSupplier.DoOnAfterListPopupFormShow(ListForm: TBaseListingGUI;
...
procedure TfrmSupplier.DoOnCheckForEmailsProgress(const msg: string;
...
procedure TfrmSupplier.DoOnGetMessageCount(const MessageCount: integer);
begin
  MailProgesss.MaxValue := MessageCount;
end;

procedure TfrmSupplier.DoOnProgressCancel(Sender: TObject);
begin
  ProgressCanceled := true;
end;

function TfrmSupplier.ChkCustReqdFields: boolean;
begin
...
procedure TfrmSupplier.cmdCustomLabelsOldClick(Sender: TObject);
begin
  OpenCustomFields;
end;
...
procedure TfrmSupplier.LetterbtnClick(Sender: TObject);
begin
...
procedure TfrmSupplier.FaxbtnClick(Sender: TObject);
begin
    if Accesslevel >= 5 then Exit;
    PostDB(qrySupplier);
    EditDB(qrySupplier);
...
procedure TfrmSupplier.EmailbtnClick(Sender: TObject);
begin
    if Accesslevel >= 5 then Exit;
...
procedure TfrmSupplier.EditbtnClick(Sender: TObject);
begin
  DisableForm;
  try
...
procedure TfrmSupplier.CNewbtnClick(Sender: TObject);
begin
  DisableForm;
  try
...
procedure TfrmSupplier.grdContactRowChanged(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.grdCorrespondenceDblClick(Sender: TObject);
var
...
procedure TfrmSupplier.grdCorrespondenceFieldChanged(Sender: TObject;
  Field: TField);
begin
  inherited;
  fbIsModified := true;
...
procedure TfrmSupplier.grdCorrespondenceRowChanged(Sender: TObject);
var
  stream: TStream;
  str: string;
...
begin
  ...
  if qryDocuments.FieldByName('MessageId').AsString <> '' then begin
    ...
    if SameText(qryDocuments.FieldByName('Ref_Type').AsString,'Email') then begin
      HTMLViewer.LoadFromString(EmailConst.EmailContentNotAvailible);
    end;
...
procedure TfrmSupplier.chkdocumentsClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.chkShowAllClick(Sender: TObject);
begin
  inherited;
  qryContacts.filtered := false;
  if not chkShowAll.Checked then begin
...
procedure TfrmSupplier.grpHistoryClick(Sender: TObject);
var
  iIndex: integer;
begin
  DisableForm;
...
procedure TfrmSupplier.HtmlViewerHotSpotClick(Sender: TObject;
...
procedure TfrmSupplier.HtmlViewerImageRequest(Sender: TObject;
...
procedure TfrmSupplier.TabCtl20Change(Sender: TObject);
var
...
procedure TfrmSupplier.cmdCopyClick(Sender: TObject);
begin
  inherited;
  qrySupplier.FieldByName('BillStreet').AsString := Street.Text;
  qrySupplier.FieldByName('BillStreet2').AsString := Street2.Text;
  qrySupplier.FieldByName('BillStreet3').AsString  := Street3.Text;
  qrySupplier.FieldByName('BillSuburb').AsString := Suburb.Text;
  qrySupplier.FieldByName('BillState').AsString := State.Text;
  qrySupplier.FieldByName('BillPostCode').AsString := PostCode.Text;
  qrySupplier.FieldByName('BillCountry').AsString := Country.Text;
end;

procedure TfrmSupplier.EmployeeIDEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.cmdNewClick(Sender: TObject);
begin
  DisableForm;
  try
...
procedure TfrmSupplier.grdDocsDblClick(Sender: TObject);
var
  FileNamedocx,FileName: OleVariant;
  sRef: string;
  form : TComponent;
...
procedure TfrmSupplier.btnInactiveFollowUpClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.SuburbCloseUp(Sender: TObject; LookupTable, FillTable: TDataSet; Modified: boolean);
begin
  if not Modified then Exit;
  inherited;
  if Modified then begin
...
procedure TfrmSupplier.SuburbEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.btnInactiveClick(Sender: TObject);
begin
  inherited;
  qryContacts.Edit;
...
procedure TfrmSupplier.cmdShippingClick(Sender: TObject);
var
...
procedure TfrmSupplier.cmdPrintClick(Sender: TObject);
var
  sSQL: string;
  sClientID: string;
begin
...
function TfrmSupplier.IsFollowUpOK: boolean;
begin
  Result := true;
...
procedure TfrmSupplier.grdFollowUpColExit(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.btnAddfollowupnotedateClick(Sender: TObject);
begin
  inherited;
      if tbOtherFollowUpNotes.ReadOnly  then exit;
      EditDB(tbOtherFollowUp);
...
procedure TfrmSupplier.grdFollowUpColEnter(Sender: TObject);
begin
  inherited;
  if sameText(grdFollowUp.GetActiveField.FieldName , tbOtherFollowUpNotes.FieldName) then begin
    if tbOtherFollowUpNotes.ReadOnly  then exit;
...
procedure TfrmSupplier.qrySupplierAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSupplier.qrySupplierAfterOpen(DataSet: TDataSet);
begin
  inherited;
  LoadMemTable;
...
procedure TfrmSupplier.LoadMemTable;
begin
...
procedure TfrmSupplier.lvAttachmentsDblClick(Sender: TObject);
var
  item: TListItem;
  Option: word;
  FileName: string;
...
procedure TfrmSupplier.qryBankCodesBeforeOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSupplier.qryContactsAfterOpen(DataSet: TDataSet);
begin
  inherited;
  LoadEditedFlagMemTable(qryContacts, memqryContacts);
end;

procedure TfrmSupplier.tbOtherFollowUpAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSupplier.tbOtherFollowUpAfterOpen(DataSet: TDataSet);
begin
  inherited;
  LoadEditedFlagMemTable(tbOtherFollowUp, memtbOtherFollowUp);
end;

procedure TfrmSupplier.qrySupplierFirstNameChange(Sender: TField);
//var newName: string;
begin
  inherited;
...
procedure TfrmSupplier.qrySupplierLastNameChange(Sender: TField);
//var newName: string;
begin
  inherited;
...
procedure TfrmSupplier.FormCloseQuery(Sender: TObject; var CanClose: boolean);
...
function TfrmSupplier.SaveOrCancelChanges: boolean;
var
  iExitResult: integer;
begin
    Result := True;
...
function TfrmSupplier.MakeSuppliercontact: boolean;
(* var
  s:String; *)
begin
...
Function TfrmSupplier.MakeNewcontactifnone:Boolean;
var
  fbShowAllL:Boolean;
begin
  REsult := False;
...
function TfrmSupplier.SAvesupplier: boolean;
...
procedure TfrmSupplier.grdContactFieldChanged(Sender: TObject; Field: TField);
begin
  inherited;
...
procedure TfrmSupplier.grdDocsFieldChanged(Sender: TObject; Field: TField);
begin
  inherited;
  fbIsModified := true;
end;

procedure TfrmSupplier.grdDocsIButtonClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.grdFollowUpFieldChanged(Sender: TObject; Field: TField);
begin
  inherited;
  fbIsModified := true;
end;

procedure TfrmSupplier.qrySupplierRepIDChange(Sender: TField);
begin
  inherited;
  qrySupplier.Edit;
...
procedure TfrmSupplier.QrySupplierskillsAfterInsert(DataSet: TDataSet);
begin
  inherited;
  PostDB(qrySupplier);
...
procedure TfrmSupplier.QrySupplierskillsAfterOpen(DataSet: TDataSet);
begin
  inherited;
...
procedure TfrmSupplier.FormKeyDown(Sender: TObject; var Key: word; Shift: TShiftState);
var
  cThisKey: char;
begin
...
procedure TfrmSupplier.btnAddDateTimeClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.btnPOClick(Sender: TObject);
var
  frm: TPurchaseGUI;
begin
  DisableForm;
...
procedure TfrmSupplier.TabCtl20Resize(Sender: TObject);
begin
  inherited;
  RealignTabControl(TabCtl20, 2);
end;

procedure TfrmSupplier.edtURLDblClick(Sender: TObject);
begin
  inherited;
  ShellExecute(0, 'open', PChar(Trim(edtURL.Text)), nil, nil, sw_shownormal);
end;

procedure TfrmSupplier.cboBankCodeEnter(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.cboDefaultAPAccountNameCloseUp(Sender: TObject; LookupTable, FillTable: TDataSet; modified: Boolean);
begin
  inherited;
  EditDB(qrySupplier);
...
procedure TfrmSupplier.cboRepChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.TabCtl20Changing(Sender: TObject; var AllowChange: boolean);
begin
  inherited;
  AllowChange := true;
  (*if not IsClientNameUnique(txtCompany.Text, KeyID) then begin
...
procedure TfrmSupplier.KeyIdChanged(const NewKeyId: integer);
begin
  logInuseform;
...
procedure TfrmSupplier.btnOtherPhonesClick(Sender: TObject);
Var
...
procedure TfrmSupplier.grdContactKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
procedure TfrmSupplier.edtBankAccNoMouseUp(Sender: TObject;
...
procedure TfrmSupplier.edtPrintNameChange(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.edtPrintNameExit(Sender: TObject);
begin
  inherited;
  if Trim(qrySupplier.FieldByname('PrintName').AsString) <> '' then PostDB(qrySupplier);
end;

procedure TfrmSupplier.SuburbQryBeforeOpen(DataSet: TDataSet);
begin
  inherited;
  SuburbQry.Params.ParamByName('xRegionID').AsInteger := AppEnv.RegionalOptions.ID;
end;

procedure TfrmSupplier.chkActiveClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.grdFollowUpDblClick(Sender: TObject);
var
  Form: TBaseInputGUI;
begin
  inherited;
...
function TfrmSupplier.GetKeyString: string;
begin
...
procedure TfrmSupplier.GlobalEventHandler(const Sender: TObject; const Event: string; const Data: Pointer);
begin
  tbOtherFollowUp.Close;
  tbOtherFollowUp.Open;
end;

procedure TfrmSupplier.tbOtherFollowUpAfterScroll(DataSet: TDataSet);
...
procedure TfrmSupplier.Callcontactform(Sender: TDNMSpeedbutton);
Var
  Form : TComponent;
begin
  inherited;
...
procedure TfrmSupplier.UpdateMe(const Cancelled: boolean;
...
procedure TfrmSupplier.wwIButton3Click(Sender: TObject);
begin
  inherited;
...
procedure TfrmSupplier.CompleteComboNotInList(const aObserver: TObject);
begin
  inherited;
  if Assigned(fLastComboAccessed) then begin
...
procedure TfrmSupplier.SetComboFocus(var Msg: TMessage);
begin
  if Assigned(fLastComboAccessed) then SetControlFocus(fLastComboAccessed);
end;

procedure TfrmSupplier.SetKeyString(const Value: string);
begin
  SetKeyString(TSupplier, qrySupplier, Value , 'Printname');
end;

procedure TfrmSupplier.SetStateParams(const Value: TJsonObject);
begin
  inherited;
...
function TfrmSupplier.ShiptoAddress: String;
begin
...
procedure TfrmSupplier.AssignComboAccessed(Sender: TObject);
begin
  if Sender is TwwDBLookupCombo then fLastComboAccessed := TwwDBLookupCombo(Sender);
end;

procedure TfrmSupplier.UpdatefromEquipment;
begin
  EditDB(qrySupplier);
...
procedure TfrmSupplier.ApplicationEvents1Exception(Sender: TObject;
  E: Exception);
begin
  inherited;
...
function TfrmSupplier.NewCompanyNameOk(const aCompanyName: string): boolean;
begin
  result := isCompanyNameOk(aCompanyName , 0);
end;
procedure TfrmSupplier.btnGetEmailsClick(Sender: TObject);
begin
  inherited;
  CheckForEmails;
end;

function TfrmSupplier.isCompanyNameOk(const aCompanyName: string;fKeyID:Integer): boolean;
begin
  result:= true;
...
procedure TfrmSupplier.qrySupplierCompanySetText(Sender: TField;
  const Text: String);
begin
  inherited;
...
procedure TfrmSupplier.TermsCloseUp(Sender: TObject; LookupTable,
  FillTable: TDataSet; modified: Boolean);
begin
  if not Modified then Exit;
  inherited;
...
procedure TfrmSupplier.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  inherited;
...
function TfrmSupplier.SupplierStatement: TBaseListingGUI;
begin
...
function TfrmSupplier.SupplierPaymentList: TBaseListingGUI;
begin
...
procedure TfrmSupplier.AccountNoKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmSupplier.AfterPopupformshow;
begin
  inherited;
...
procedure TfrmSupplier.QrySupplierCalcFields(DataSet: TDataSet);
begin
  inherited;
  QrySuppliercDocumentPath.AsString  := cDocumentPath(QrySupplier);
end;
procedure TfrmSupplier.btnSetDocumentPathClick(Sender: TObject);
begin
  SetDocPath(QrySupplier);
end;
procedure TfrmSupplier.btnOpenDocumentPathClick(Sender: TObject);
begin
  inherited;
...
initialization
  RegisterClassOnce(TfrmSupplier);
  with FormFact do begin
    RegisterMe(TfrmSupplier, 'TSupplierListGUI_*=ClientID');
    RegisterMe(TfrmSupplier, 'TSupplierExpressListGUI_*=ClientID');
    RegisterMe(TfrmSupplier, 'TSupplierSkillListGUI_*=ClientID');
    (*RegisterMe(TfrmSupplier, 'TRemindersListGUI_*_Supplier FollowUp=ClientID');*)
    RegisterMe(TfrmSupplier, 'TReminderListGUI_*_Supplier FollowUp=ClientID');
    (*RegisterMe(TfrmSupplier, 'TRemindersListGUI_Client_AppointmentSupplier=ClientID');*)
    RegisterMe(TfrmSupplier, 'TReminderListGUI_Client_AppointmentSupplier=ClientID');
    RegisterMe(TfrmSupplier, 'TAppointmentListGUI_Company_Supplier=ClientID');
    RegisterControl(TfrmSupplier, 'TPurchaseGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TPurchaseGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TReturnAGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TBillGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TCreditGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TfrmCheque_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TSmartOrderGUI_cboClientR=ClientID');
    RegisterControl(TfrmSupplier, 'TfrmPayrollCompanyInfo_cboClient=ClientID');
    RegisterControl(TfrmSupplier, '*_cboSupp=ClientID');
    RegisterControl(TfrmSupplier, '*_cboSupplier=ClientID');
    RegisterControl(TfrmSupplier, 'TfmSuppPayments_cboClient=ClientID');
    RegisterControl(TfrmSupplier, 'TfmSuppPrepayments_cboClient=ClientID');
    //RegisterControl(TfrmSupplier, 'TFixedAssetGUI_cboClient=ClientID');
    RegisterControl(TfrmSupplier, 'TfmFixedAsset_cboClient=ClientID');
    RegisterControl(TfrmSupplier, 'TfrmEmployee_cboSuperFund=ClientID');
    RegisterControl(TfrmSupplier, 'TfrmEmployee_Workcover=ClientID');
    RegisterMe(TfrmSupplier, 'TMarketingLeadsGUI_Company_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TGlobalListSearchFrm_*_Supplier(Contact)=ID');
    RegisterMe(TfrmSupplier, 'TGlobalListSearchFrm_*_Supplier(Primary Contact)=ID');
    RegisterMe(TfrmSupplier, 'TGlobalListSearchFrm_*_Supplier=ID');
    RegisterMe(TfrmSupplier, 'TGlobalListFrm_*_Supplier(Contact)=ID');
    RegisterMe(TfrmSupplier, 'TGlobalListFrm_*_Supplier=ID');
    RegisterMe(TfrmSupplier, 'TMarketingContactListGUI_*_S=ClientID');
    RegisterMe(TfrmSupplier, 'TClientListGUI_*_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TContactListGUI_*_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TGlobalsearchGUI_*_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TOtherfollowupsGUI_*_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TClientMainContactsGUI_*_Supplier=ClientID');
    RegisterMe(TfrmSupplier, 'TCustomFieldValuesSupplierGUI_*_MasterID=ClientID');
    RegisterMe(TfrmSupplier, 'TCustomFieldValuesListContactGUI_*_Supplier=ClientId');
    RegisterMe(TfrmSupplier, 'TCustomFieldValuesListSupplierGUI_*=ClientId');
  end;

finalization
  UnRegisterClass(TfrmSupplier);
end.
