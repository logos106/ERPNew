unit InventoryAssetCheckListByType;
...
type
  TInventoryAssetCheckListByTypeGUI = class(TInventoryAssetCheckListGUI)
  private
  Protected
...
{$R *.dfm}
{ TInventoryAssetCheckListByTypeGUI }

procedure TInventoryAssetCheckListByTypeGUI.InitformSQL;
begin
  inherited;
    Qrymain.sql.add('Select ');
    Qrymain.sql.add('convert(3, UNSIGNED)  as Details ,  ');
    ...
    Qrymain.sql.add('from ' + tablename +'  group by Type');
    Qrymain.sql.add('Order by Type, Details, ProductName, AccountName');
end;

initialization
  RegisterClassOnce(TInventoryAssetCheckListByTypeGUI);
