unit InventoryAssetCheckListByProduct;
...
type
  TInventoryAssetCheckListByProductGUI = class(TInventoryAssetCheckListGUI)
  private
  Protected
...
{$R *.dfm}
{ TInventoryAssetCheckListByProductGUI }

procedure TInventoryAssetCheckListByProductGUI.InitformSQL;
begin
  inherited;
  Qrymain.sql.add('Select ');
    Qrymain.sql.add('convert(1, UNSIGNED)  as Details , ');
  ...
  Qrymain.sql.add('from ' +tablename +' group by Productname');
  Qrymain.sql.add('Order by Productname, Details, Accountname, Type');
end;

initialization
  RegisterClassOnce(TInventoryAssetCheckListByProductGUI);
