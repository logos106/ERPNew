...
type
  TfrmAllowances = class(TBaseInputGUI)
    ...
    cboAllowanceCode: TwwDBComboBox;
...
procedure TfrmAllowances.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  Closequeries;
  Action := caFree;
...
procedure TfrmAllowances.btnCancelClick(Sender: TObject);
begin
  inherited;
  if fsModal in FormState then  else Close;
end;

procedure TfrmAllowances.FormShow(Sender: TObject);
begin
  DisableForm;
  try
    try
...
procedure TfrmAllowances.btnNewClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmAllowances.cboBasedOnNotInList(Sender: TObject; LookupTable: TDataSet; NewValue: string; var Accept: boolean);
begin
  Accept := false;
end;

procedure TfrmAllowances.AddNewAllowanceDetails;
begin 
  Allowance.AccountName := GetAccountName(AppEnv.PayPrefs.AllowancePaidAccountID);
  Allowance.AccountID   := AppEnv.PayPrefs.AllowancePaidAccountID;
...
procedure TfrmAllowances.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmAllowances.btnSaveClick(Sender: TObject);
begin
  inherited;
  Allowance.PostDB;
...
procedure TfrmAllowances.qryAllowanceAccountNameChange(Sender: TField);
begin
  inherited;
  Allowance.AccountID := GetAccountID(qryAllowance.FieldByName('AccountName').AsString);
end;

procedure TfrmAllowances.UpdateNotes;
begin
...
procedure TFrmAllowances.DoBusinessObjectEvent( const Sender: TDatasetBusObj; const EventType, Value: string);
begin
  inherited;
...
procedure TfrmAllowances.cboCalcByCloseUp(Sender: TwwDBComboBox;
  Select: Boolean);
begin
...
procedure TfrmAllowances.cboDefaultAccountCloseUp(Sender: TObject;
...
procedure TfrmAllowances.cboDisplayInChange(Sender: TObject);
begin
  inherited;
...
initialization
  RegisterClassOnce(TFrmAllowances);
  with FormFact do begin
    RegisterMe(TfrmAllowances, 'TfrmAllowanceList_*=AllowanceID');
    RegisterControl(TfrmAllowances, '*_cboAllowance=AllowanceID');
  end;
end.
