...
type
  ...
  TSFTPClient = class;
  TPMIThreadTask = class(TServerModuleThreadTask)
  private
...
//    function ProcessFile(stream: TStream; report: TStringList; reportOnly: boolean = true): boolean;
    ...
    function ProcessOrder(POId: integer): boolean;
...
//function TPMIThreadTask.CheckForNewFiles: boolean;
//var
//  x: integer;
//  qry: TERPQuery;
//  cmd: TERPCommand;
...
constructor TPMIThreadTask.Create;
begin
  inherited;

end;

destructor TPMIThreadTask.Destroy;
begin
  XMLDoc := nil;
  PO.Free;
  FSFTPClient.Free;
...
procedure TPMIThreadTask.DoWork;
begin
  inherited;
...
function TPMIThreadTask.FTPParamsOk: boolean;
var
  KeyFileName: string;
begin
  result := false;
...
procedure TPMIThreadTask.Log(const msg: string;
...
function TPMIThreadTask.ProcessOrder(POId: integer): boolean;
var
  node, IDOC, line: IXMLNode;
  fileName: string;
  qry: TERPQuery;
begin
  result := false;
  if not Assigned(PO) then begin
  ...
  try
    try
      qry.SQL.Text := 'select SupplierProductCode from tblparts where PartsId = :PartsID';
      PO.Load(POId);
      if PO.Lock then begin
        try
          XMLDoc.XML.Clear;
          XMLDoc.Active := true;
//          XMLDoc.Version := BusObjConst.XMLDocVersion;
//          XMLDoc.Encoding := BusObjConst.XMLDocEncoding;
          node := XMLDoc.AddChild('ORDERS05');
          IDOC := node.AddChild('IDOC');

          node := IDOC.AddChild('E1EDK01');
          node.AddChild('KZABS').NodeValue := 'X';

          node := IDOC.AddChild('E1EDK14');
          node.AddChild('QUALF').NodeValue := '012';
          node.AddChild('ORGID').NodeValue := 'ZEDI';

          node := IDOC.AddChild('E1EDK14');
          node.AddChild('QUALF').NodeValue := '007';
          node.AddChild('ORGID').NodeValue := '14';

          node := IDOC.AddChild('E1EDK14');
          node.AddChild('QUALF').NodeValue := '006';
          node.AddChild('ORGID').NodeValue := '20';

          node := IDOC.AddChild('E1EDK14');
          node.AddChild('QUALF').NodeValue := '008';
          node.AddChild('ORGID').NodeValue := 'AU01';

          node := IDOC.AddChild('E1EDK14');
          node.AddChild('QUALF').NodeValue := '011';
          node.AddChild('ORGID').NodeValue := '1687';

          {  Order Creation date in the format YYYYMMDD }
          node := IDOC.AddChild('E1EDK03');
          node.AddChild('IDDAT').NodeValue := '022';
          node.AddChild('DATUM').NodeValue := FormatDateTime('yyyymmdd',PO.OrderDate);

          { Global Location Number }
          node := IDOC.AddChild('E1EDKA1');
          node.AddChild('PARVW').NodeValue := 'AG';
          node.AddChild('PARTN').NodeValue := GLN;

          { Purchase Order Number }
          node := IDOC.AddChild('E1EDK02');
          node.AddChild('QUALF').NodeValue := '001';
          node.AddChild('BELNR').NodeValue := IntToStr(PO.PurchaseOrderId);

          { PO Lines }
          PO.Lines.First;
          while not PO.Lines.EOF do begin
            line := IDOC.AddChild('E1EDP01');
            line.AddChild('POSEX').NodeValue := IntToStr( PO.Lines.Dataset.RecNo * 10 );
            line.AddChild('MENGE').NodeValue := FloatToStr(PO.Lines.UOMQtySold);

            node := Line.AddChild('E1EDP19');
            node.AddChild('QUALF').NodeValue := '003';
            { GTIN (Global Trade Item Number) }
//            node.AddChild('IDTNR').NodeValue := PO.Lines.ProductName; //  PO.Lines.PartBarcode;
            qry.ParamByName('PartsID').ASInteger := PO.Lines.ProductID;
            qry.Open;
            node.AddChild('IDTNR').NodeValue := qry.ParamByName('SupplierProductCode').AsString;
            qry.Close;
          ...
          { Footer }
          node := IDOC.AddChild('E1EDS01');
          node.AddChild('SUMID').NodeValue := '001';
          node.AddChild('SUMME').NodeValue := IntToStr(PO.Lines.Count);

          fileName := '';
          if POOutputDir <> '' then begin
            fileName := IncludeTrailingPathDelimiter(POOutputDir) + 'PO_' + IntToStr(PO.ID) + '_' + FormatDateTime('yyyymmdd-hhnnss',now) + '.xml';
            XMLDoc.SaveToFile(fileName);
            Log('Processed PO ' + IntToStr(PO.ID)+ ' and saved to file: ' + fileName,ltInfo);
          end;

          PO.OrderStatus := ProcessedStatus;
          PO.PostDb;
        ...
        Log('Unable to lock PO ' + IntToStr(PO.ID) + ' at this time, will try again later',ltWarning);
      end;
    except
      on e: exception do begin
        Log('Error processing PO '+IntToStr(PO.ID)+': ' + e.Message,ltError);
...
procedure TPMIThreadTask.ProcessOrders;
var
  qry: TERPQuery;
...
begin
//  result := false;
  Log('Checking for orders that need to be processed ..', ltInfo);
...
function TPMIThreadTask.RequirementsOk: boolean;
begin
  result := false;
...
//function TPMIThreadTask.ProcessFile(stream: TStream; report: TStringList;
...
//function TPMIThreadTask.ProcessFiles: boolean;
//var
//  qry: TERPQuery;
//  stream: TMemoryStream;
//  msg: string;
...
//                Log('Unable to download file ' + qry.FieldByName('FileName').AsString, ltError);
//                exit;
//              end;
...
function TPMIThreadTask.SFTPClient: TSFTPClient;
begin
