...
type
  ...
  private
    ...
    procedure MakeAdward(CampaignId, Adward, adwardID: String);
    procedure MakeCampaign(CampaignId, Campaign: String);
  public
    constructor Create;
  ...
  try
    for AccIdx := 0 to Accounts.Count -1 do begin
      ...
      if not Acc.O['fields'].B['Active'] then
        continue;
      AccountID :=Acc.O['fields'].S['AccountID'];
      AccountOk := true;
      for ProfileIdx := 0 to Acc.O['fields'].A['Profiles'].Count -1 do begin
        ...
        While (dt1 < dt2) and AccountOk do begin
      ...
      for EntryIdx := 0 to InData.A['data'].Count -1 do begin
        ...
        try
          AnalyticsDb.GetObject('TGaData',GaData,DbConn,
            'where ProfileId = ' + QuotedStr(Profile.O['fields'].S['ProfileId']) +
            ' and DateTime = "' + FormatDateTime('yyyy-mm-dd hh:nn:ss',aDate + (OneHour * Entry.ItemByName['ga:hour'].Value.AsInteger))+ '"' +
            ' and Keyword = ' + QuotedStr(StrToQueryStr(Entry.S['ga:keyword'])) +
            ' and country = ' + QuotedStr(Entry.S['ga:country']) +
            ' and Browser = ' + QuotedStr(Entry.S['ga:browser']) +
            ' and OperatingSystem = ' + QuotedStr(Entry.S['ga:operatingSystem']) );
          ...
          on e: exception do begin
            result:= false;
            msg:= 'Error saving Analytics data: ' + e.Message + #13#10 +
              '(' + ReplaceStr(GaData.AsString,#13#10,'') + ')';
            exit;
          end;
        end;
      end;
...
Procedure TGoogleAnalytics.MakeCampaign(CampaignId, Campaign:String);
begin
  AnalyticsDb.MakeCampaign(DBConn, AccountID, CampaignId, Campaign);
end;

Procedure TGoogleAnalytics.MakeAdward(CampaignId, Adward, adwardID: String);
begin
  AnalyticsDb.MakeAdward(DBConn, AccountID, CampaignId, Adward, adwardID);
end;
      ...
      for EntryIdx := 0 to InData.A['data'].Count -1 do begin
        ...
        try
          AnalyticsDb.GetObject('TGadataforCost',GaData,DbConn,
                                  ' where ProfileId = "' + (*Profile.O['fields'].S['*)Entry.S['ProfileId'] + '"' +
          ...
          if GaData.O['fields'].I['UId'] = 0 then begin
            GaData.O['fields'].S['ProfileId']:= (*Profile.O['fields'].S['*)Entry.S['ProfileId'];
          ...
          AnalyticsDb.Save(GaData,DbConn);
          MakeCampaign(Entry.S['ga:adwordsCampaignId'], Entry.S['ga:Campaign']);
          MakeAdward(Entry.S['ga:adwordsCampaignId'], Entry.S['ga:adwordsAdGroupId'] , Entry.S['ga:adGroup']);
        except
          on e: exception do begin
            result:= false;
            msg:= 'Error saving Analytics data: ' + e.Message + #13#10 +
      ...
      for EntryIdx := 0 to InData.A['data'].Count -1 do begin
        ...
        try
          AnalyticsDb.GetObject('TGaDataForCharts',GaData,DbConn,
            'where ProfileId = "' + (*Profile.O['fields']*)Entry.S['ProfileId'] + '"' +
            ' and DateTime = "' + FormatDateTime('yyyy-mm-dd hh:nn:ss',aDate )+ '"' +
            ' and language = ' + QuotedStr(Entry.S['ga:language']) +
            ' and Location = ' + QuotedStr(Entry.S['ga:country']) +
            ' and Keyword = ' + QuotedStr(StrToQueryStr(Entry.S['ga:keyword']))  +
            ' and dayOfWeek = ' + QuotedStr(Entry.S['ga:dayOfWeek']) +
            ' and Browser = ' + QuotedStr(Entry.S['ga:browser']) +
            ' and Campaign = ' + QuotedStr(Entry.S['ga:campaign']) );

          if GaData.O['fields'].I['UId'] = 0 then begin
            GaData.O['fields'].S['ProfileId']:= Entry.S['ProfileId']; //Profile.O['fields'].S['ProfileId'];
            GaData.O['fields'].DT['DateTime']:= aDate;
...
//            '_' +  Profile.O['fields'].S['ProfileId'] + '_' +  Entry.S['ga:keyword'] + '.txt');
          ...
          on e: exception do begin
            result:= false;
            msg:= 'Error saving Analytics Impressions data: ' + e.Message + #13#10 +
              '(' + ReplaceStr(GaImpressions.AsString,#13#10,'') + ')';
            exit;
          end;
        end;
      end;
