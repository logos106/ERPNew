...
begin
  hrs:= StrToInt(Copy(edtInterval.Text,1,2));
  mins:= StrToInt(Copy(edtInterval.Text,4,2));
  secs:= StrToInt(Copy(edtInterval.Text,7,2));
  result:= EncodeTime(hrs,mins,secs,0);
end;

procedure TfmDatabaseEdit.SetCheckInterval(const Value: TDateTime);
begin
  edtInterval.Text:= FormatDateTime('hh:nn:ss',Value)
end;
...
begin
  ...
  mins:= StrToInt(Copy(edtInterval.Text,4,2));
  secs:= StrToInt(Copy(edtInterval.Text,7,2));
  if (hrs = 0) and (mins = 0) and (secs < 5) then begin
