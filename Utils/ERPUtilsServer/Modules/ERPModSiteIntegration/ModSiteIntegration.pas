unit ModSiteIntegration;

interface

uses
  ServerModuleObj, JsonObject, BusObjSchemaLib, ApiObjectListObj,
  WebApiClientObj, classes, ConfigDbObj, SynchReportObj, LogMessageTypes,
  LogThreadBase;

type

  TModSiteIntegration = class(TServerModule)
  private
    Config: TJsonObject;
    ...
    procedure SetSourceLastChangeTime(aClassName: string;
...
//    function GetModuleVersion: string; override;
    ...
    function GetLoggingLevel: TLoggingLevel; override;
...
uses
  ...
  Math, SystemLib, UURIEncode, ModuleUtils, JsonToObject, JsonRPCUtils,
  JsonRPCConst, LogThreadDbLib;

const
  TypesToSynch: array[0..73] of string =
    (
     'TERPPreference',
     'TCustomFieldList',
     'TCustomFieldConfig',
...
procedure TModSiteIntegration.AfterLoadConfig;
begin
  inherited;

end;

constructor TModSiteIntegration.Create;
begin
  inherited;
//  self.LoadConfig;
...
function TModSiteIntegration.CreateNewLogger: TLoggerBase;
begin
  result := TDbLogger.Create(nil);
end;

procedure TModSiteIntegration.DecLevel;
begin
  if Level > 0 then
    Dec(Level);
end;

destructor TModSiteIntegration.Destroy;
begin
  SchemaList.Free;
  SourceLists.Free;
...
procedure TModSiteIntegration.DoCheck;
var
  SiteIdx: integer;
  Site: TJsonObject;
  msg: string;
...
begin
  ...
  try
    ...
    Logger.SetLoggingLevel(TLoggingLevel(Config.I['LoggingLevel']));

    for SiteIdx := 0 to Config.A['Sites'].Count -1 do begin
      Site:= Config.A['Sites'].Items[SiteIdx].AsObject;
      Log('Checking "' + Site.S['Name'] + '" ...',ltDetail);
      if Site.B['Active'] and (Site.DT['NextSyncTime'] <= now) then begin
        try
          if (Site.O['Interval'].O['DayOfTheWeek'].B[IntToStr(DayOfTheWeek(now))]) then begin
            ...
            Report.Init(Site);
            dt:= now;
            Log('Processing "' + Site.S['Name'] + '" ...',ltInfo);
            if DoSync(Site, msg) then
              Log('Finished processing "' + Site.S['Name'] + '" (time taken: ' + FormatDateTime('hh:nn:ss',now-dt)+ ')',ltInfo)
            else begin
            ...
            Log('Site synch not due to be run today.',ltDetail)
          end;
        finally
        ...
        if not Site.B['Active'] then
          Log('Site not active.',ltDetail)
        else
          Log('Site synch not due until: ' + FormatDateTime('yyyy-mm-dd hh:nn:ss',Site.DT['NextSyncTime']),ltDetail);
      end;
...
procedure TModSiteIntegration.DoOnCommandMessage(aData: string;
  ctx: TJsonObject);
begin
  inherited;
...
procedure TModSiteIntegration.DoOnCommandRequest(aRequest: string;
...
procedure TModSiteIntegration.DoOnMessage(aClientId: integer; aData: string;
  ctx: TJsonObject);
begin
  inherited;
...
procedure TModSiteIntegration.DoOnMessage(aClientId: integer; aData,
  ctx: TJsonObject);
begin
  inherited;
...
procedure TModSiteIntegration.DoOnRequest(aClientId: integer; aRequest, ctx,
...
function TModSiteIntegration.DoSync(Site: TJsonObject;
...
function TModSiteIntegration.DoSyncList(aClassName: string;
  var msg: string): boolean;
var
  s: string;
...
procedure TModSiteIntegration.DoOnRequest(aClientId: integer; aRequest: string;
...
procedure TModSiteIntegration.DoWork;
begin
  inherited;
  if (Now >= fNextCheckTime) then begin
    Log('',ltDetail);
    Log('Checking for site integrations that need to be processed ..',ltDetail);
    DoCheck;
    Log('Check finished.',ltDetail);
  end;
end;

function TModSiteIntegration.GetActive: boolean;
var
  j: TJsonObject;
begin
  j := JO;
  try
    j.AsString:= ConfigDb.Value;
    result := j.B['Active'];
  finally
    j.Free;
  end;
...
function TModSiteIntegration.GetKeyStringPropertyName(aClassName: string;
  var aKeyStringPropertyName, msg: string): boolean;
var
  aSchema: TBOSchema;
begin
  result:= false;
...
function TModSiteIntegration.GetLoggingLevel: TLoggingLevel;
var
  j: TJsonObject;
begin
  j := JO;
  try
    j.AsString:= ConfigDb.Value;
    result := TLoggingLevel(j.I['LoggingLevel']);
  finally
    j.Free;
  end;
end;

function TModSiteIntegration.GetModuleDescription: string;
begin
  result := 'Module to integrate different ERP sites';
end;

//function TModSiteIntegration.GetModuleVersion: string;
//begin
//  result := '11.0.0.0';
//end;

function TModSiteIntegration.GetSchema(aClassName: string;
...
function TModSiteIntegration.GetSourceLastChangeTime(
  aClassName: string): TDateTime;
begin
...
procedure TModSiteIntegration.IncLevel;
begin
  Inc(Level);
end;

function TModSiteIntegration.ListSourceTimeStampNewer(ObjSrc,
...
procedure TModSiteIntegration.LoadConfig(encrypt: boolean);
//var
...
function TModSiteIntegration.MappRefObj(Src, Dst, RefSchema: TJsonObject;
...
//        GetKeyStringPropertyName(RefSchema.S['type'],RefItemKeyStringProperty, msg);
        ...
        if DonePropList.IndexOf(Prop) < 0 then begin
          if not Dst.O['Fields'].Exists(Prop) then begin
            Dst.O['Fields'].Add(Prop,Src.O['Fields'].ItemByName[Prop].ValueType);
          end;
...
procedure TModSiteIntegration.SaveConfig(encrypt: boolean);
begin
  inherited;

end;

procedure TModSiteIntegration.SetActive(const Value: boolean);
var
  j: TJsonObject;
begin
  j := JO;
  try
    j.AsString:= ConfigDb.Value;
    j.B['Active'] := Value;
    ConfigDb.Value := j.AsString;
  finally
    j.Free;
  end;
end;

procedure TModSiteIntegration.SetLoggingLevel(const Value: TLoggingLevel);
var
  j: TJsonObject;
begin
  j := JO;
...
procedure TModSiteIntegration.SetSourceLastChangeTime(aClassName: string;
  const Value: TDateTime);
begin
...
function TModSiteIntegration.SourceTimeStampNewer(ObjSrc,
  ObjDst: TJsonObject): boolean;
var
  ValRel: TValueRelationship;
begin
...
function TModSiteIntegration.SupObjListSyncDone(aClassName: string;
  var msg: string): boolean;
begin
...
function TModSiteIntegration.SyncItem(aClassName: string; aId: integer;
  var msg: string): boolean;
var
...
//  NewGlobRef: string;
  ...
  function CheckGlobalRefAndUpdateSiteCode(var aSrcObj: TJsonObject; var aMsg: string): boolean;
  var
    json, jsonRslt: TJsonObject;
  begin
    result:= true;
        ...
        if GetSchema(Src.S['type'], Sch, subMsg) then begin
          for i := 0 to Src.O['fields'].Count -1 do begin
            ...
            if (not Dst.O['fields'].Exists(PairSrc.Name)) then begin
              Dst.O['fields'].Add(PairSrc.Name,PairSrc.ValueType);
            end;
//            if (not Dst.O['fields'].Exists(PairSrc.Name)) then begin
...
//            if (PairSrc.Value.ValueType = valObject) and
//              (PairSrc.Value.AsObject.S['type'] =  'TShippingMethod') then
//                Log('',ltdebug);
...
procedure TModSiteIntegration.UpdateConfigNextUpdateTime(SiteName: string;
