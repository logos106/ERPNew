...
type
  ...
  public
    ...
    function CheckReplies(var msg, ReplyInfo: string; MaxReplies: integer = 100; Confirm: boolean = false): boolean; override;
...
uses
  SysUtils, MessageMediaConst, Classes, URILib, JsonObject, Variants;

{ TSMSMsgMed }

function TSMSMsgMed.CheckReplies(var msg, ReplyInfo: string; MaxReplies: integer; Confirm: boolean): boolean;
var
    ...
    if result then begin
      ...
      if Assigned(node) then begin
        if node.Attributes['returned'] > '0' then begin
          json := JO;
          try
            json.I['returned'] := node.Attributes['returned'];
            json.I['remaining'] := node.Attributes['remaining'];
...
//    XMLOut.SaveToFile('c:\temp\message.xml');
