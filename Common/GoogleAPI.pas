...
type
  TAuthRec = class(TObject)
  public
    Email: string;
    Auth: string;
  end;

  TAuthList = class(TObject)
  private
    fList: TObjectList<TAuthRec>;
    function GetItemByEmail(aEmail: string): TAuthRec;
  public
    constructor Create;
    destructor Destroy; override;
    property ItemByEmail[aEmail: string]: TAuthRec read GetItemByEmail; default;
    function Add(aEmail, aAuth: string): TAuthRec;
    procedure Delete(aEmail: string);
  end;
  ...
  private
    ...
    function AddAuth(aEmailAddress, aPassword: string): TAuthRec;
    procedure Reconnect;
  public
    constructor Create;
    destructor Destroy; override;
  ...
  private
    ...
    fPrimaryCalendarEvents: TCalendarEventList;
    function GetPrimaryCalendarEvents: TCalendarEventList;
    function GtTaskList: TTaskLists;
    function GetCalendarList: TCalendarList;
    procedure DoOnConnectDisconnect(Sender: TObject);
  public
    constructor Create;
    destructor Destroy; override;
    property Connection: TGoogleAPIConnection read fConnection;
    property TaskLists: TTaskLists read GtTaskList;
    property CalendarList: TCalendarList read GetCalendarList;
    property PrimaryCalendarEvents: TCalendarEventList read GetPrimaryCalendarEvents;
  end;
  ...
  public
    ...
    function get(aId: string): TCalendarEvent;
  end;
...
uses
  DateTimeUtils, SysUtils, URILib, StringUtils, dialogs, GoogleConst, HTTPConst;
...
function TAuthList.Add(aEmail, aAuth: string): TAuthRec;
begin
  result := TAuthRec.Create;
  result.Email := aEmail;
  result.Auth := aAuth;
  fList.Add(result);
end;

constructor TAuthList.Create;
begin
  fList := TObjectList<TAuthRec>.Create(true);;
end;

procedure TAuthList.Delete(aEmail: string);
var
  a: TAuthRec;
begin
  for a in fList do begin
   if a.Email = aEmail then begin
     fList.Delete(fList.IndexOf(a));
     break;
   end;
  end;
end;

destructor TAuthList.Destroy;
begin
  fList.Free;
  inherited;
end;

function TAuthList.GetItemByEmail(aEmail: string): TAuthRec;
var
  a: TAuthRec;
begin
  result := nil;
...
function TGoogleAPIConnection.AddAuth(aEmailAddress,
  aPassword: string): TAuthRec;
var
  sl, authSL: TStringList;
begin
  fAuthList.Delete(aEmailAddress);
  ...
  try
    ...
    try
      ...
      result := fAuthList.Add(aEmailAddress,sl.Values['Auth']);
    finally
...
var
  a: TAuthRec;
begin
  if Connected and (aEmailAddress = fEmailAddress) and (aPassword = fPassword) then begin
    result := true;
    exit;
  end;
  fConnected := false;
  ...
  a := fAuthList[aEmailAddress];
  if not Assigned(a) then
    a := self.AddAuth(aEmailAddress, aPassword);

  fClient.Request.CustomHeaders.Values['Authorization'] := 'GoogleLogin auth=' + a.Auth;
  fConnected := true;
...
begin
  ...
  try
    ...
    if fClient.ResponseCode = 401 then begin
      Reconnect;
      fClient.Delete(AddKey(aURL));
    end
    else if fClient.ResponseCode = 410 then begin
      { 410 = Gone (ie already deleted) .. do nothing }
    end
    else
      raise;
  end;
...
begin
  ...
  try
    ...
    if (fClient.ResponseCode = 401) or (fClient.ResponseCode = 403)  then begin
      Reconnect;
      result := fClient.Get(AddKey(aURL));
    end
    else if (fClient.ResponseCode = 410) then begin
...
procedure TGoogleAPIConnection.Reconnect;
begin
  fAuthList.Delete(fEmailAddress);
  fConnected := false;
  Connect(fEmailAddress, fPassword);
end;
...
begin
  ...
  fConnection.OnConnect := DoOnConnectDisconnect;
  fTaskLists := nil;
  fCalendarList := nil;
  fPrimaryCalendarEvents := nil;
end;
...
begin
  fTaskLists.Free;
  fConnection.Free;
  fPrimaryCalendarEvents.Free;
  inherited;
end;

procedure TGoogle.DoOnConnectDisconnect(Sender: TObject);
begin
  FreeAndNil(fTaskLists);
  FreeAndNil(fCalendarList);
  FreeAndNil(fPrimaryCalendarEvents);
end;

function TGoogle.GetCalendarList: TCalendarList;
begin
  if not Assigned(fCalendarList) then
    fCalendarList := TCalendarList.Create(Connection);
  result := fCalendarList;
end;

function TGoogle.GetPrimaryCalendarEvents: TCalendarEventList;
begin
  if not self.Connection.Connected then
    raise Exception.Create(Self.ClassName + ' - Not Connected');

  if not Assigned(fPrimaryCalendarEvents) then begin
    fPrimaryCalendarEvents := TCalendarEventList.Create(Connection,Connection.EmailAddress);
  end;
  result := fPrimaryCalendarEvents;
end;

function TGoogle.GtTaskList: TTaskLists;
begin
  if not Assigned(fTaskLists) then
    fTaskLists := TTaskLists.Create(Connection);
  result := fTaskLists;;
end;
...
begin
  result := RFC3339StrToDateTime(data.S['updated']);
end;
...
begin
  ...
  if Assigned(tl) then begin
    Data.A['list'].Delete(tl.Data);
    fList.Delete(fLIst.IndexOf(tl));
  end;
end;
...
begin
  result := RFC3339StrToDateTime(Data.S['completed']);
end;
...
begin
  result := RFC3339StrToDateTime(Data.S['due']);
end;
...
begin
  fOwner := aOwner;
  inherited Create(aOwner.Connection);
...
function TTasks.get(aId: string): TTask;
var
  j: TJsonObject;
begin
  result := self.TaskById(aId);
  if not Assigned(result) then begin
    result := TTask.Create(self.fOwner.Connection);
    j := JO;
    try
      j.AsString := Connection.Get('https://www.googleapis.com/tasks/v1/lists/' + URILib.PercentEncode(self.fOwner.id) + '/tasks/' + aId);
      Data.A['items'].Add(J);
      result.Data := J;
      fLIst.Add(result);
    except
...
//  List; { force loading of list }
  j := JO;
  if aTask.Data.StringExists('id') and (aTask.Data.S['id'] = '') then
    aTask.Data.Delete('id');
...
procedure TTasks.SetUpdatedMin(const Value: TDateTime);
begin
  FUpdatedMin := Value;
end;
...
begin
  ...
  if (fDataFunc.StringExists('date')) and (fDataFunc.S['date'] <> '') then
    result := Trunc(RFC3339StrToDateTime(fDataFunc.S['date']));
end;
...
begin
  ...
  if (fDataFunc.StringExists('dateTime')) and (fDataFunc.S['dateTime'] <> '') then
    result := RFC3339StrToDateTime(fDataFunc.S['dateTime']);
end;
...
begin
  result := RFC3339StrToDateTime(Data.S['created']);
end;
...
function TCalendarEventList.get(aId: string): TCalendarEvent;
var
  j: TJsonObject;
begin
  result := self.EventById(aId);
  if not Assigned(result) then begin
    result := TCalendarEvent.Create(self.Connection);
    j := JO;
    try
      j.AsString := Connection.Get('https://www.googleapis.com/calendar/v3/calendars/' + URILib.PercentEncode(self.fCalendarId) + '/events/' + aId);
      Data.A['items'].Add(J);
      result.Data := J;
      fLIst.Add(result);
    except
...
begin
//  List; { force loading of list }
  if Data.StringExists('id') and (Data.S['id'] = '') then
    Data.Delete('id');
  j := JO;
