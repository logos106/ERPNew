...
uses
  Pipes, SyncObjs, windows, classes, ThreadProperty,
...
//  RequestStatus := rsIdle;
  ...
  if fIsServer then begin
    Server := TPipeServer.CreateUnowned; //  TPBPipeServer.Create('\\.\pipe\' + fChannelName)
    Server.PipeName := fChannelName;
    Server.OnPipeMessage := DoOnPipeMessage;
    Server.Active := true;
  end
  else begin
    Client := TPipeClient.CreateUnowned; //  TPBPipeServer.Create('\\.\pipe\' + fChannelName + 'Client');
    Client.PipeName := fChannelName;
    Client.ServerName := '.';
    Client.OnPipeMessage := DoOnPipeMessage;
  end;
...
procedure TModuleComms.DoOnPipeMessage(Sender: TObject; Pipe: HPIPE;
  Stream: TStream);
var
  ss: TStringStream;
begin
      ...
      try
        ...
        if Assigned(Client) then begin
          if not Client.Connected then
            Client.Connect();
          result := client.SendStream(stream);
        end
        else begin
          for x := 0 to Server.ClientCount -1 do
            Server.SendStream(Server.Clients[x],stream);
        end;
      finally
        stream.Free;
      end;
