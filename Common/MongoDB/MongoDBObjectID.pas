unit MongoDBObjectID;

interface
(*
  ObjectId is a 12 byte value made up as follows
...
*)
  ...
  function MongoObjectIdStr: string;  { thread safe unique id string }
  function MongoObjectId: string;
  function MongoObjectIdToStr(aMongoObjectId: string): string;
...
var
  mongoObjectID_MachineID,
  mongoObjectID_Counter: integer;
...
function mongoObjectIdToStr(aMongoObjectId: string): string;
begin
  result := Copy(aMongoObjectId,Length(bsonObjectIDPrefix)+1,
    Length(aMongoObjectId)-Length(bsonObjectIDPrefix)-Length(bsonObjectIDSuffix){-1});
end;

function MongoObjectIdStr: string;
var
  st:TSystemTime;
  a,b,c,d:integer;
const
