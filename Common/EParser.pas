...
type
  ...
  TParserEMessage=(pBadQuotes, pBadBrackets, pBadSyntax, pUnknownDataType);
  ...
  TGetVarEvent = procedure (const VariableName: string; var Value: Variant) of object;
...
//    FTokenType := ttQuote;
    if Assigned(fOnGetVar) then begin
      fOnGetVar(fToken,aValue);
      if not VarIsNull(aValue) then begin
        if VarIsStr(aValue) then begin
          FToken := aValue;
          FTokenType := ttString;
        end
        else if VarIsNumeric(aValue) then begin
          FToken := aValue;
          FTokenType := ttNumber;
        end
        else if VarIsType(aValue, varBoolean) then begin
          FTokenType := ttString;
          if aValue = true then
            FToken := 'True'
          else
          ...
          raise TParserException.CreatePE(pUnknownDataType);
      end;
    end
    else
      raise TParserException.CreatePE(pUnknownDataType);
...
begin
  Res^.Res := trBool;
  if Op='=' then begin
    if StringIsNumber(Res^.Value) and StringIsNumber(Res2^.Value) then begin
      if StrToFloat(Res^.Value)=StrToFloat(Res2^.Value) then
        Res^.Value := STrue
      else
        Res^.Value := SFalse;
    end
    else if StringIsBoolean(Res^.Value) and StringIsBoolean(Res2^.Value) then begin
      if StrToBool(Res^.Value)=StrToBool(Res2^.Value) then
        Res^.Value := STrue
      else
        Res^.Value := SFalse;
    end
      ...
      { assume string type }
      if LowerCase(Res^.Value)=LowerCase(Res2^.Value) then
        Res^.Value := STrue
      else
        Res^.Value := SFalse;
    end;
